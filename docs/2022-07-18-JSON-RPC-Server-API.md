...
- `antecedent` is `\\bottom` => `satisfiable = True` and `condition = \\bottom`
- `consequent` is `\\bottom` => `satisfiable = False` and `condition = \\bottom`
- implication holds => `satisfiable = True` and `condition /= \\bottom`
