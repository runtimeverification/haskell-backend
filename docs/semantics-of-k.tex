...
and digits and the character~``\verb|'|'' that starts with an 
alphabet and is not a \syntacc{keyword}.
...
All meta-level sorts are non-parametric sorts.
...
Heads are user-definable constructors of \syntacc{pattern} (see 
Section~\ref{sec:kore-syntax-patterns}).
In practice, a head is either a symbol or an alias.
The only difference between symbols and alias are their semantics.
...
\begin{grammar}\small
<definition> ::= <attribute> `module' <declaration>$^+$ `endmodule'
...
<meta-symbol-declaration> <attribute>

<object-symbol-declaration> ::= `symbol' <object-head> `(' <object-sort-list> 
`)' `:' <object-sort> <attribute>

<meta-symbol-declaration> ::= `symbol' <meta-head> `(' <meta-sort-list> `)' 
`:' <meta-sort> <attribute>

<alias-declaration> ::= <object-alias-declaration> | <meta-alias-declaration> 
<attribute>

<object-alias-declaration> ::= `alias' <object-head> `(' <object-sort-list> 
`)' `:' <object-sort> <attribute>

<meta-alias-declaration> ::= `alias' <meta-head> `(' <meta-sort-list> `)' 
`:' <meta-sort> <attribute>
...
The language defined by the above BNF grammar forms a superset of the Kore 
language.
For a Kore definition to be valid, it has to satisfy some 
\emph{wellformedness} conditions.
...
\quottt{\sharpsymbol} \ s \ \quottt{:} \ \quottt{\KSort\{\}} & if $s$ is an 
object 
sort 
variable 
$S$ \\
...
Sometimes we want to lift the list to 
one meta-pattern of the sort \textup{\KSortList}, instead of
a list of meta-patterns of the sort \textup{\KSort}.
For that reason, we define the following {list-lifting process}
\begin{align*}
...
\subsubsection{Lift Sort Declarations to Symbol and Axiom Declarations}
...
\begin{align*}
  &\ \denote{\texttt{sort\{$\dots$\} \parametric{$C$}{$S_1$,\dots,$S_n$}}} \\
  = &\ \texttt{symbol 
  $\denote{C}$($\underbrace{\parametric{
  ...
  \texttt{$\denote{\parametric{C}{S_1,\dots,S_n}}$,}
  \\
  &\qquad\qquad\qquad\quad
  \texttt{\parametric{\Ksort}{}($\denote{C}_\name$, 
  $\denote{
  ...
  S}}(\denote{\parametric{C}{S_1,\dots,S_n}}))
\end{align*}
...
\subsubsection{Lift Object Symbol and Alias Declarations to Symbol and Axiom 
Declarations}
...
An object alias declaration is lifted in the same way as an object symbol 
declaration is lifted, except that the alias declaration does not have the two 
axioms.
