...
However, decades of years of research not only proved that order-sorted 
algebras are intuitive, convenient, and easy to use in many practical cases, 
but also showed their power does come for a cost (and sometimes a big cost).
...
in sort $s'$.
We fake this intuition of inclusion by defining an injective partial function 
called the \emph{injection function}:
\begin{center}
	\begin{tabular}{rl}
		$\inj{s}{s'} \colon s \rightharpoonup s'$
		& \doubleslash parametric sort defined for any $s \preceq s'$ \\
		$\inj{s}{s}(x) = x$
...
%\end{align*}
%
\subsubsection{Subsort Overloading}
