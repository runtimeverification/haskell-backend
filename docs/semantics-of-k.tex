\documentclass[UTF8,11pt]{article}
  ...
  \newcommand{\Ksymbol}{\texttt{\sharpsymbol symbol}}
  \newcommand{\KSymbolceil}{\texttt{\sharpsymbol Symbolceil}}
  ...
  \newcommand{\KnilKPatternList}{\texttt{\sharpsymbol nilKPatternList}}
  \newcommand{\KconsKPatternList}{\texttt{\sharpsymbol consKPatternList}}
  \newcommand{\KappendKPatternList}{\texttt{\sharpsymbol appendKPatternList}}
  \newcommand{\KinKPatternList}{\texttt{\sharpsymbol inKPatternList}}
  \newcommand{\KdeleteKPatternList}{\texttt{\sharpsymbol deleteKPatternList}}
  \newcommand{\KSortList}{\texttt{\sharpsymbol SortList}}
  \newcommand{\KnilKSortList}{\texttt{\sharpsymbol nilKSortList}}
  \newcommand{\KconsKSortList}{\texttt{\sharpsymbol consKSortList}}
  \newcommand{\KappendKSortList}{\texttt{\sharpsymbol appendKSortList}}
  \newcommand{\KinKSortList}{\texttt{\sharpsymbol inKSortList}}
  \newcommand{\KdeleteKSortList}{\texttt{\sharpsymbol deleteKSortList}}
  \newcommand{\KSymbolList}{\texttt{\sharpsymbol SymbolList}}
  \newcommand{\KinKSymbolList}{\texttt{\sharpsymbol inKSymbolList}}
  \newcommand{\KnilKCharList}{\texttt{\sharpsymbol nilKCharList}}
  \newcommand{\KconsKCharList}{\texttt{\sharpsymbol consKCharList}}
  \newcommand{\KVariableList}{\texttt{\sharpsymbol VariableList}}
  \newcommand{\KnilKVariableList}{\texttt{\sharpsymbol nilKVariableList}}
  \newcommand{\KconsKVariableList}{\texttt{\sharpsymbol consKVariableList}}

  \newcommand{\KVariable}{\texttt{\sharpsymbol Variable}}
  \newcommand{\KVariableToKPattern}{\texttt{\sharpsymbol VariableToKPattern}}
  ...
  \newcommand{\KgetFvs}{\texttt{\sharpsymbol getFvs}}
  \newcommand{\KgetFvsFromPatterns}{\texttt{\sharpsymbol getFvsFromPatterns}}
  ...
  \newcommand{\Kdeduce}{\texttt{\sharpsymbol provable}}
  ...
  \newcommand{\syntacc}[1]{$\langle$\textit{#1}$\rangle$}
...
This section is a follow-up to Section~\cref{sec:finite-mechanisms},\improvement{we should uniformly use either ``cref'' of ``ref''}
in which 
...
Predicate symbols in $K$ are defined in later sections when they firstly appear.
\begin{notation}
...
In the meta-theory of matching logic $K$, the symbol $\KSymbolceil$ is 
the 
...
$$
the sort $\XList$ is in $S_K$, and the following functional constructors of the 
sort $\XList$ are in $\Sigma_K$
\begin{align*}
...
This fact is captured in $K$ by the next axiom
$$
...
A theory declares some sorts, so in $K$ there is a predicate symbol
$$ \KsortDeclared \colon \KSort \to \KPred$$
A theory declares some symbols, so in $K$ there is a predicate symbol
...
A theory declares some patterns as axioms, so in $K$ there is a 
predicate symbol
...
\begin{center}
	...
	\end{tabular}
\end{center}
What Kore does is to give users an elegant way to specify $\denote{T}$ without
getting exposed to too much of the meta-theory infrastructure.
...
\subsubsection{Lexicon and Syntactic Categories}
...
\paragraph{Kore Keywords.} 
The following keywords are used to start a Kore 
declaration.
\begin{center}
	{\ttfamily
...
Identifiers are nonempty sequences of printable characters.
An identifier is said to be a \emph{meta identifier}, if it starts with the 
letter ``\verb|#|''.
If it starts with the letter ``\verb|\|'', it is said to be a \emph{reserved 
identifier}.
Otherwise, it is said to be an \emph{object identifier}.

\begin{grammar}\small\centering
	<id>    ::= <meta-id> | <reserved-id> | <object-id> 
\end{grammar}

Identifiers are building blocks of many syntactic categories in Kore, such as 
sorts, symbols, variables, and patterns.
It is important to separate meta-level identifiers from object-level ones, 
because Kore provides users to write a mixture of the meta-level and the 
object-level, and we do not want to confuses the meta-level with the 
object-level.

\paragraph{Sort.}
In Kore, all sorts are parametric.
A sort is constructed by ``\verb|_{_}|'', which takes a \emph{sort 
constructor} and a list of \emph{sort parameters}.
Sort parameter can be either a sort, or a \emph{sort variable}.
A sort variable itself is a sort, too.
For example,
\begin{center}{\ttfamily
\begin{tabular}{lclcl}
	\textrm{Sort} & \textrm{Sort Constructor} & \textrm{Sort Parameter(s)} & 
	\textrm{Sort Schema?} & \textrm{Depending Variable(s)}
	\\\hline
	\parametric{Nat}{} & Nat & - & \textrm{No} & -
	\\\hline
...
\end{center}
A sort is said to be an \emph{object sort} if the sort constructor does not 
start with ``\verb|#|'', in which case we called it an \emph{object sort 
constructor}.
Object sort constructor can be declared with the ``\verb|sort|'' keyword, 
followed by the name of the object sort constructor being defined and a list of 
object sort variables wrapped with curly braces.

\begin{grammar}\small\centering
	<object-sort-constructor-declaration> ::= `sort' <object-id> `{' 
	<object-sort-variable-list> `}'
\end{grammar}
If an object sort constructor is declared with no sort variable, we say it is 
\emph{non-parametric} or \emph{atomic}. 
Otherwise, we say it is \emph{parametric}.
We say a parametric object  sort constructor is \emph{$n$-parametric} if it is 
declared with $n$ sort variables.
...
A sort is said to be a \emph{meta sort} if the sort constructor starts with 
``\verb|#|'', in which case we called the constructor a \emph{meta sort 
constructor}.
In Kore, the following non-parametric meta sort constructors are predefined:
\begin{center}
	{\ttfamily
		\begin{tabular}{llll}
...
\end{center}
Meta sort constructors are predefined. New meta sort constructors \emph{cannot} 
be declared.
\emph{Meta sort variables} are meta identifiers, for example, \verb|#S|, 
\verb|#S'|.
Meta sort variables can only be instantiated with (the above eight) meta sorts, 
and 
object sort 
variables can only be instantiated with (potentially infinite) object sorts.

\paragraph{Symbol.}
In Kore, all symbols are parametric.
A symbol is constructed by ``\verb|_{_}|'', which takes a \emph{symbol 
constructor} and a list of sorts. 
If any sort variable appears in a symbol, we say the symbol is a \emph{symbol 
schema}.
...
\begin{center}{\ttfamily
		\begin{tabular}{lclcl}
			...
			\\\hline
			\parametric{succ}{\parametric{}{}} & succ & - & 
			\textrm{No} & -
			\\\hline
...
\emph{Object symbol constructors} are symbol constructors which are object 
identifiers.
\emph{Meta symbol constructors} are symbol constructors which are meta 
identifiers.
Both symbol constructors can be declared with the keyword ``\verb|symbol|'', 
followed by an (object or meta) identifier, a list of (object or meta) sort 
variables, a list of (object or meta) argument sorts, and an (object or meta) 
return sort.
\begin{grammar}\small
	<object-symbol-constructor-declaration> ::= \quad
	\alt `symbol' <object-id> `{' 
		<object-sort-variable-list> `}' `(' <object-sort-list> `)' `:' 
		<object-sort>
		
	<meta-symbol-constructor-declaration> ::= \quad
	\alt `symbol' <meta-id> `{' 
...
In Kore, all logic connectives are sort parametric, constructed with 
``\verb|_{_}|''.
Logic connective constructor are reserved identifiers.

\begin{grammar}\small
	<logic-connective> ::= <object-logic-connective> | <meta-logic-connective>
	
	<object-logic-connective> ::= <reserved-id> `{' <object-sort-list> `}'
	
	<meta-logic-connective> ::= <reserved-id> `{' <meta-sort-list> `}'
\end{grammar}
...
\paragraph{Pattern.}
In Kore, patterns have a highly unified construction (apart from two 
exceptions: string literals and variable patterns, which will be introduced 
separately).
A pattern is constructed with ``\verb|_(_)|'', which takes a \emph{head} and a 
list of patterns.
Heads can be either a symbol or a logic connective.
We call a head is an \emph{object (or meta) head}, if it is an object (or meta) 
symbol or an object (or meta)
logic connective.
Patterns constructed with object (or meta) heads are \emph{object (or meta) 
patterns}.
Notice that whether a pattern is object or meta only depends on its head, not 
the patter list that the head applies on.
Both object and meta heads can take a list of mixed patterns.
\begin{grammar}\small
...
Variable patterns are constructed with ``\verb|_:_|'', which takes an 
identifier and a sort.
If the sort is an object sort, we say the variable pattern is an \emph{object 
variable}.
Otherwise, it is a \emph{meta variable}.
\begin{grammar}\small
	<pattern> ::= <variable>
	
	<variable> ::= <object-variable> | <meta-variable>
	
    <object-variable> ::= <id> `:' <object-sort>
    
    <meta-variable> ::= <id> `:' <meta-sort>
\end{grammar}

String literals are considered as meta patterns.
\begin{grammar}\small
	<pattern> ::= <string-literals>
\end{grammar}

\paragraph{Axiom Declarations.}
...
Give a theory $T$ defined as a Kore definition ``\verb|definition.kore|'' which consists of a set of object sort declarations, a set of object and meta symbol declarations, and a set of object and meta axiom declarations using the syntax that we have introduced before.
...
The Kore definition ``\verb|#definition.kore|'' will contain only meta symbol declarations and meta axiom declarations.
The lifting algorithm is defined in~\ref{alg:lifting}.
...
\subsubsection{Distinguish Meta-Level and Object Level}
In Kore, there is always a clear distinction between object level targets and 
meta level targets.

\subsubsection{Lift Object Identifiers To String Literals}
Object identities are lifted to string literals by wrapping them with quotation 
marks.
We denote this lifting function as $\denote{\_}_\name$.

\subsubsection{Lift Object \& Reserved Identifiers To Meta Identifiers}
Object and reserved identifiers are lifted to meta identifiers by appending a 
``\texttt{\sharpsymbol}'' in front, denoted as 
$\denote{\_}_\sharp$.

\subsubsection{Lift Logic Connective Constructors To Meta Symbols}
Logic connectives constructor are lifted to non-parametric meta symbols as 
follows.
For any logic connective constructor $C_\itlogic$, 
$$\denote{C_\itlogic} = \parametric{\denote{C_\itlogic}_\sharp}{}$$
\subsubsection{Lift Object Sorts}
Object sort constructors $C_\itsort$ are lifted to non-parametric meta symbols 
as
$$\denote{C_\itsort} = \parametric{\denote{C_\itsort}_\sharp}{}$$
For example
\begin{center}{\ttfamily
\begin{tabular}{lcr}
    \textrm{Object Sort Constructor} & \textrm{Is Lifted To} & 
    \textrm{Non-Parametric Meta-Symbol}
    \\\hline
...
\end{center}
A sort variable $\tt S$ is lifted to a variable pattern {\tt \sharpsymbol 
$\tt S\cln \KSort$}.
A sort $\parametric{C_\itsort}{s_1,\dots,s_n}$ with sort constructor 
$C_\itsort$ and a list of 
sort parameters is lifted as
$$ \denote{\parametric{C_\itsort}{s_1,\dots,s_n}} = 
\texttt{$\denote{C_\itsort}$($\denote{s_1}$,\dots,$\denote{s_n}$)}$$

For example,
\begin{center}{\ttfamily
		\begin{tabular}{llr}
			\textrm{Object Sort} & \textrm{Is Lifted To} &  
			\textrm{Meta-Pattern}
			\\\hline
...
For any object sort declaration \begin{center}
	\texttt{sort 
$\parametric{C_\itsort}{S_1,\dots,S_n}$} 
\end{center}
with sort constructor $C_\itsort$ and sort variables $S_1,\dots,S_n$, 
define the lifting operation as
\begin{align*}
  \denote{\texttt{sort \parametric{$C_\itsort$}{$S_1$,\dots,$S_n$}}} =
  &\ \texttt{symbol 
  $\denote{C_\itsort}$(\parametric{\KSort}{},\dots,\parametric{\KSort}{}):\parametric{\KSort}{}}
  \\
  &\ \texttt{\parametric{axiom}{\sharpsymbol S} 
  \parametric{equals}{\parametric{\KSort}{},\sharpsymbol S}(}
  \\
  &\qquad\qquad\qquad
  \texttt{$\denote{C_\itsort}$($\denote{S_1}$,\dots,$\denote{S_n}$),}
  \\
  ...
  $\denote{S_1,\dots,S_n}_\llist$))}
\end{align*}
The sort list lifting operation $\denote{\_}_\llist$ takes a list 
$\alpha$ of sorts 
and lift them to a $\parametric{\KSortList}{}$ pattern as follows (using Kore 
syntax):
\begin{align*}
\denote{\alpha}_\llist = 
\begin{cases*}
\parametric{\KnilKSortList}{}\texttt{()} & if $\alpha$ is empty list \\
\parametric{\KconsKSortList}{}\texttt{($\denote{s}$,$\denote{\beta}_\llist$)} 
& if 
$\alpha 
= s, \beta$
\end{cases*}
\end{align*}
\subsubsection{Lift Object Symbols}
Object symbol constructors $C_\itsymbol$ are lifted to non-parametric meta 
symbols as
$$ \denote{C_\itsymbol} = \parametric{\denote{C_\itsymbol}_\sharp}{}$$
...
$\denote{C_\itsymbol}$($\underbrace{\parametric{\KSort}{},\dots,
	\parametric{\KSort}{}}_\text{$m$\,times}$,$\underbrace{
	\parametric{\KPattern}{},\dots,\parametric{
		\KPattern}{}}_\text{$n$\,times}$):\parametric{\KPattern}{}}
\\ 
& \texttt{\parametric{axiom}{\sharpsymbol S}
...
& \qquad\qquad\qquad \texttt{
	\parametric{\Kapplication}{}($\denote{
		C_\itsymbol}_\itsymbol$,$\denote{\texttt{\sharpsymbol 
			P1:\parametric{\KPattern}{},\dots,\sharpsymbol 
			Pn:\parametric{\KPattern}{}}}_\llist$))
}
\end{align*}
in which $\denote{\_}_\itsymbol$ (given $S_1,\dots,S_m,s_1,\dots,s_n,s$) is 
defined as
\begin{align*}
  \denote{C_\itsymbol}_\itsymbol = 
  &\ \texttt{\Ksymbol($\denote{C_\itsymbol}_\name$,} \\
  &\ \qquad\qquad \texttt{$\denote{S_1,\dots,S_m}_\llist$,} \\
  &\ \qquad\qquad \texttt{$\denote{s_1,\dots,s_n}_\llist$,} \\
  &\ \qquad\qquad \texttt{$\denote{s}$))}
\end{align*}
\subsubsection{Lift Patterns}
Recall the following grammar of patterns
\begin{grammar}
	<pattern> ::= \quad
	\alt <id> `:' <sort>
	\alt <string-literal>
	\alt <head> `(' <pattern-list> `)'
\end{grammar}
Recall that the syntactic category \syntacc{head} is either a logic connective
or a symbol, which is always of the form $\parametric{C}{s_1,\dots,s_n}$ with 
constructor $C$ and a list of sorts as parameters $s_1,\dots,s_n$.

In Kore, a pattern $\varphi$ is lifted as follows.
\begin{align*}
\varphi = 
\begin{cases*}
\Kvariable(\denote{x}_\name, \denote{s}) & if $\varphi$ is \texttt{$x$:$s$} and 
$s$ is an object sort\\
\denote{C}(\denote{s_1},\dots,\denote{s_n},\denote{
	\varphi_1},\dots,\denote{\varphi_n})&
 if $\varphi$ is $\parametric{C}{s_1,\dots,s_n}(\varphi_1,\dots,\varphi_n)$ \\
 &and 
 $C$ is an object symbol or logic connective constructor\\
 \parametric{C}{s_1,\dots,s_n}(\denote{
 	\varphi_1},\dots,\denote{\varphi_n})&
 if $\varphi$ is $\parametric{C}{s_1,\dots,s_n}(\varphi_1,\dots,\varphi_n)$ \\
 &and 
 $C$ is a meta symbol or logic connective constructor\\
 \varphi & if $\varphi$ is a meta variable or a string literal
\end{cases*}
\end{align*}
\subsubsection{Lifting Axiom Declarations}
In Kore, an axiom declaration
\begin{center}\ttfamily
	\parametric{axiom}{$s_1$,\dots,$s_n$,$s'_1$,\dots,$s'_m$}
	$\varphi$
\end{center}
with object sort variables $s_1,\dots,s_n$, meta sort variables 
$s'_1,\dots,s'_m$ and a pattern $\varphi$ is lifted as follows.

\begin{align*}
\denote{\texttt{\parametric{axiom}{$s_1$,\dots,$s_n$,$s'_1$,\dots,$s'_m$}
	$\varphi$}} =
\begin{cases*}
\texttt{\parametric{axiom}{$s'_1$,\dots,$s'_m$} \Kdeduce($\denote{\varphi}$)} & 
if $\varphi$ is an object variable \\&or has an object head\\
\texttt{\parametric{axiom}{$s'_1$,\dots,$s'_m$} $\denote{\varphi}$} & if
$\varphi$ is a meta variable, \\&a string literal, \\&or has a meta head
\end{cases*}
\end{align*}
...
The following are reserved keywords in Kore:
\begin{center}
\texttt{
\begin{tabular}{lllll}
	and & or & implies & iff & equals \\ 
	mem & contains & ceil & floor & not \\ 
	exists & forall & top & bottom &  \\ 
	Kand & Kor & Kimplies & Kiff & Kequals \\ 
	Kmem & Kcontains & Kceil & Kfloor & Knot \\ 
	Kexists & Kforall & Ktop & Kbottom &  \\ 
	KSort & KSymbol & KVariable & KPattern & KPred \\ 
	KSortList & KSymbolList & KVariableList & KPatternList &  \\ 
	Ksort & Ksymbol & Kvariable & KvariableAsPattern &  \\ 
	KnilKSortList & KconsKSortLiat & KnilKSymbolList & KconsKSymbolList & KnilKVariableList \\ 
	KconsKVariableList & KnilKPatternList & KconsKPatternList & KgetSort & Kprobable \\ 
	KsortDeclared & KsymbolDeclared & KaxiomDeclared &  &  \\ 
\end{tabular} 
}
\end{center}
...
Therefore, the sorts $\KPred$, $\KString$, $\KSymbol$, $\parametric{\KSort}{}$, 
and 
