...
The main focus in this section is not about \emph{how to design and implement the prover $P$}, but about \emph{how to encode $\varphi$ and $T$ as data that can be fed into $P$ as inputs}.
...
\begin{itemize}
	\item developing a reflective theory $K$ of matching logic;
	\item axiomatizing the mechanisms in the reflective theory $K$.  
\end{itemize}
...
The following table shows the difference of topics that we are interested between a theory of arithmetics and its reflective theory:
\begin{center}
\begin{figure}[h]
\centering
\begin{tabular}{|l|l|}
\hline
\textbf{The theory of arithmetic} & \textbf{The reflective theory}
\\ \hline
Is multiplication commutative?  & Is ``$x \times y = y \times x$'' derivable?
\\ \hline
Is $233$ a prime number? & Does ``$\mathit{isPrime(\varphi)}$'' has sort $\mathit{Bool}$ if $\varphi$ has sort $\mathit{Nat}$?
\\ \hline
The de-factorization of $12$ is $2 \times 2 \times 3$.
&
The DNF of $\varphi \to \psi$ is $\varphi' \vee \psi'$, \\
& where $\varphi'$ and $\psi'$ are DNFs of $\neg \varphi$ and $\psi$, respectively.
\\ \hline
\end{tabular}
\caption{Different concerns of topics in a theory and its reflective theory.}
\end{figure}
\end{center}
...
\subsection{Meta-Theory of Matching Logic: Finite Case}
...
\subsubsection{Matching Logic Sorts and Symbols}
...
\subsubsection{Matching Logic Patterns}
...
\subsubsection{Matching Logic Theories}
...
Start from this subsection, we are going to define matching logic theories as 
...
\subsubsection{Matching Logic Proof System}
...
\KgetFvs(\varphi)) \to \Kdeduce(T, \overline{x \cln s_1 \in_{s_1}^{s_2} 
\varphi}).
\end{equation*}
...
\KinKPatternList(\overline{x \cln s_1}, \KgetFvs(\varphi)) \to \Kdeduce(T, 
\varphi).
\end{equation*}
...
\subsection{Matching Logic Theory of Matching Logic: With Meta-Variables}
