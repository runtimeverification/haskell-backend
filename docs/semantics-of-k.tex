...
\end{align*}
The symbol schema $$\parametric{\KinXList}{s} \colon 
\texttt{\sharpsymbol}\ttX \times \XList \to s \quad \text{for any 
$s \in S_K$}$$
defines for each sort $s$ in the meta-theory $K$, a predicate symbol that
checks list membership and returns either $\top_s$ or $\bot_s$:
\begin{align*}
  & \neg \parametric{\KinXList}{s}(x, \KnilXList)
  \\
  & \parametric{\KinXList}{s}(x, \KconsXList(y, L)) = (x = y) \vee 
  \parametric{\KinXList}{s}(x, L)
\end{align*}
