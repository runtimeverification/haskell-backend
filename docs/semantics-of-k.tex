...
\begin{displayquote}
	...
	subject matter.
\end{displayquote}
Following this definition, the theory $K$ is said to be a {reflective} one 
because 
(1) $K$ itself is a matching logic theory and 
(2) one is able to study and discuss the properties of matching logic itself, 
instead of the properties of some models or algebras that are axiomatized in 
the logic, in the meta-theory $K$. Some typical example properties of matching logic include the 
wellformedness of a pattern and the derivability of a pattern in a theory.

Before we put our effort in exhaustively defining the meta-theory $K$, we would 
like to firstly point how the meta-theory $K$ helps us in having a finite 
representations of patterns and theories.

\subsection{Representing Sorts, Symbols, and Patterns by Their Abstract Syntax 
Trees}
...
A naming function is a function from an object to its \emph{mention}---\emph{a 
string representation in $K$}.
A naming is a prerequisite for anything to be encoded and represented in $K$.
What matters is to have a naming function.
It does not matter which naming function we choose, as long as distinct 
objects have distinct names, which is referred to as the \emph{unique name 
principle}.

\paragraph{Abstract Syntax Trees}
As soon as we have a naming function, we can encode sorts, symbols, and 
patterns by constructing terms in $K$ that stand for the abstract syntax trees 
of them.
...
In the following, we use the theory of lambda calculus as an example to illustrate how the meta-theory $K$ works.
\subsubsection*{Example: Lambda Calculus Represented in $K$}
...
A naming function is needed to encode lambda expressions as abstract syntax trees.
We use the next naming function
\begin{center}
\begin{tabular}{c|c}
	Object targets & Names in $K$
	\\\hline
	$\Exp$ & ``Exp''
	\\\hline
	$\lambda_0$ & ``lambda0''
...
We list all declarations in $L$ and their corresponding assertions in $K$, without explaining them.
We will formally define the process in Section~\cref{?}, but for now we only provide a taste of flavor of the meta-theory $K$.
...
\begin{center}
	\begin{tabular}{l}
		...
		$\mathit{KSymbollambda0} = \Ksymbol(\text{``Exp''}, (\KExp, \KExp), \KExp)$ \\
		$\KsymbolDeclared(\mathit{KSymbollambda0})$ \\
		$\Klambdazero \colon \KPattern \times \KPattern \to \KPattern$ is a symbol\\
		$\Klambdazero(\varphi, \psi) = \Kapplication(\mathit{KSymbollambda0}, (\varphi, \psi))$\\
	\end{tabular}
\end{center}

In $L$
\begin{center}
	$\lambda$ is an alias defined as $\lambda(x, e) = \exists x . \lambda_0(x, e)$
\end{center}
...
\begin{center}
	...
	$\Klambda(v, \varphi) = \Kexists(\KExp, \KExp, v, \Klambdazero(v, \varphi))$
	\end{tabular}
\end{center}
