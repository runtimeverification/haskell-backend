...
One may argue: ``we do not have a formal semantics for (a), i.e., matching 
logic, in~\cite{rosu-2017-lmcs} anyway, so why bother to give formal semantics 
to (b)?''
...
Therefore, we can afford to write a comprehensive paper about matching logic, 
in natural languages, and educate our \K developers once and for all, but we 
cannot afford to write for each theory such a comprehensive informal document 
and educate our \K developers whenever we change the theory.
...
And we did find one such one-step solution called \emph{the meta-theory $K$.}
