...
\begin{notation}
	As a convention, we use $F$ for $\KPatternList$ variables as it appears in $\Ktheory(\Sigma, F)$, which is often abbreviated as $( \Sigma, F )$. The theory $( ( S, \Sigma ), F )$ is often abbreviated to just $( S, \Sigma, F )$, too. We use $T$ for $\KTheory$ variables.  
\end{notation}
...
In the following, we list all the inference rules of the matching logic proof system and the correspondent axioms of $\Kdeduce$. \improvement{We should mention here that all the subterms / subpatterns that appear in the axioms are assumed to be well-formed. We omit them when writing the axioms because writing all of them down need a lot of space, and will make the axioms less readable.}
