...
\improvement{This table is far from complete, but I don't have time to work on 
this right now. I need to work on revising Section 8.}
	\begin{longtable}{l|l}
...
\begin{center}
	...
	\end{tabular}
\end{center}
The theory $S_L$ also gets some axioms (schemas).
...
meta-representations are still often too lengthy to fit in one line or 
even one page.
To ease the pain of reading and writing huge meta-representations, we define 
the following two helper functions in $\denote{T}$ that allow us to write 
...
\paragraph{Represent Sort Definitions.}
...
This fact is important and should be represented.
In the meta-theory, we have a predicate symbol $\KsortDeclared$ 
that does 
...
S_K$}$$
The theory defines $\parametric{\List}{s}$ for any sort $s$ that it defines.
This is axiomatized as
$$ \forall s . \left( 
\underbrace{\parametric{\KsortDeclared}{\shs}(s)}_\text{if $s$ is defined} \to 
\underbrace{\parametric{\KsortDeclared}{\shs}(\KqList(s))}_\text{then 
$\parametric{\List}{s}$ is defined} \right) \qquad \text{for any 
$\shs \in S_K$}$$

\subsection{Represent Symbols}
Just like sorts, symbols have their meta-representations, 
too.
Recall we defined the constructor 
\begin{equation*}
\Ksymbol \colon {\underbrace{\KString}_\text{name}}\ 
\times 
\underbrace{\KSortList}_\text{sort parameters} 
...
\begin{alignat*}{2}
\Ksymbol(
&\qquottt{zero}, &&\quad\text{\doubleslash name of $O$} \\
...
\paragraph{Represent Symbol Definitions.}
...
\underbrace{\Ksymbol(\qquottt{plus}, \KnilKSortList, (\KqNat,\KqNat), 
	...
	\!\Nat\times\Nat\to \Nat$})
\end{align*}
For parametric symbols, we define axioms
\begin{align*}
& \forall s . \left(
  \underbrace{\parametric{\KsortDeclared}{\shs}(s)}_\text{if $s$ is defined} \to
  \underbrace{\parametric{\KsymbolDeclared}{\shs}(
