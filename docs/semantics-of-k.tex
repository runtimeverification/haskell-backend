...
we define matching logic theories with built-ins as a tuple
...
a set of axioms $A$, the semantics of built-ins are given
by an implementation/oracle,
and whether a pattern $\varphi_\builtin$ over the syntax of built-in sorts 
and symbols belongs to the theory $(S_\builtin, \Sigma_\builtin, S, \Sigma, A)$
is told by the oracle.
For example, let $S_\builtin = \{ \Nat \}$
and $\Sigma_\builtin = \{ 0, 1, +, \times \}$,
and let the oracle be the natural numbers with addition and multiplication.
Notice the set of patterns $\varphi$ of sort $\Nat$ in the theory
...
at the object-level into meta-level axioms about \Kdeduce.
Specifically, 
whenever we need a property $\varphi$ about built-ins, say, natural numbers,
we can assume that
$\KprovableP{\shs}(\denote{\varphi})$
is provable in the meta-theory, where
$\denote{\varphi}$ is the meta-representation of $\varphi$.
