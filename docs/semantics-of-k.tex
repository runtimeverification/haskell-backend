...
\paragraph{Sort.}
In Kore, every sorts are parametric.
...
sort variables wrapped with curly braces.
...
\paragraph{Symbol}
In Kore, every symbol is parametric.
...
\begin{center}{\ttfamily
		\begin{tabular}{lclcl}
			...
			\textrm{Parameter(s)} & 
			\textrm{Symbol Schema?} & \textrm{Depending Variable(s)}
			\\\hline
			...
			\textrm{Yes} & S
			\\\hline
			\parametric{Map}{S1,\parametric{List}{S2}} & Map & 
			S1,\parametric{List}{S2} 
			& \textrm{Yes} & S1,S2
			\\\hline
			S & - & - & \textrm{Yes} & S
		\end{tabular}
...
Object symbol constructor and meta symbol constructor.

Symbol declarations.

\paragraph{Connectives}
In Kore, logic connectives are sort parametric.

Logic connective constructor are identifiers starts with ``\verb|\|''.

\begin{grammar}\small\centering
	<logic-connective> ::= <reserved-id> `{' <sort-list> `}'
\end{grammar}

\paragraph{Pattern}\quad

\begin{grammar}\small\centering
	<pattern> ::= <variable-name> `:' <sort> | <head> <sort-list>  `(' 
	<pattern-list> `)'
\end{grammar}
