...
%\circ\;\sigma(\varphi_1,\dots,\varphi_n)
...
abstract data types.
Many algebraic specifications use \emph{subsorts} as a mechanism to model 
subtypes.
In \K, an order-sorted front-end syntax is used to allow more intuitive and 
compact \K definitions.
However, the convenience of order-sorted algebras does not come for free. 
Algorithms and decision procedures for order-sorted algebras have been proposed 
and implemented in languages like Maude, but in general order-sorted algebras 
are hard from a theoretical point of view and reasoning in an order-sorted 
setting is even harder.
Therefore, we allow back-ends to make use of order-sorted decision procedures 
and algorithms, but at the same time we want to have a crystal clear and easier 
mathematical foundation for order-sorted algebras, via a methodological 
embedding in matching logic\footnote{
It is well-known that order-sorted algebras can be reduced to many-sorted 	
algebras~\cite{GOGUEN1992217}.}.

\info{Xiaohong: how to formulate the problem so that it helps translate 
order-sorted \K definitions to many-sorted Kore definition?}
...
Subsort overloading is the main focus when it comes to order-sorted signatures 
...
In order-sorted algebras, there is a standard approach that introduces for each 
connected component $[s]$ a top sort (or called a \emph{kind}), often denoted 
as just $[s]$.
Therefore for any $s \in [s]$, $s \preceq [s]$.
And for each operator $f \colon s_1 \times \dots \times s_n \to s$, we define a 
...
\subsubsection{Embedding Order-Sorted in Matching Logic}
...
Suppose $s'_1 \preceq s''_1 \in [s_1], \dots, s'_n \preceq s''_n \in [s_n]$ and
\begin{align*}
& \parametric{f}{s'_1,\dots,s'_n} \in \Sigma_{s'_1,\dots,s'_n,s'} \\
& \parametric{f}{s''_1,\dots,s''_n} \in \Sigma_{s''_1,\dots,s''_n,s''}
\end{align*}
By definition, $s' \preceq s'' \in [s]$ and thus there is an injection function 
$\inj{s'}{s''} \colon s' \to s''$.
The following property should hold
$$
\inj{s'}{s''}(
\parametric{f}{s'_1,\dots,s'_n}(x_1 \cln s'_1,\dots,x_n \cln s'_n))
=
\parametric{f}{s''_1,\dots,s''_n}(
...
\subsection{Mimicking Order-Sorted Theories}
\label{sec:ML-OS}
...
Section~\ref{sec:dealing-with-OS} and give axiomatization for these embeddings.
The intuition is that reasoning in an order-sorted theory can be seen as 
reasoning in a many-sorted theory plus some symbols and axioms that capture the 
...
\subsubsection{The Partial Order Relation over Sorts}

In order-sorted algebras, the set of sorts $(S, \preceq)$ forms a partial order 
set and the subsort relation~$\preceq$ is a partial order relation.
To capture the subsort relation in order-sorted algebras, we define a predicate 
symbol
$$
...
The parametric predicate symbol $\Ksubsort{\shs}$ can be easily element-wisely 
extended to lists 
of sorts.
...
\Ksubsorts{\shs}(S, S')
\end{align*}
where $s, s'$ are variables of sort $\KSort$ and $S, S'$ are variables of sort 
$\KSortList$.

The subsort relation is a congruence relation
\begin{align*}
...
the readers again of the reflection property that we firstly introduced 
...
called \emph{lifting process}, which is giving Kore a semantics as a 
specification 
...
\begin{itemize}
\item An \syntacc{object-identifier}, which is a nonempty sequence of letters,
digits, the character~``\verb|'|'', and the character~``\verb|-|'' that starts 
with a
letter and is not a \syntacc{keyword}.
\item A \syntacc{meta-identifier},
which is either the character ``\verb|#|'' followed by an 
\syntacc{object-identifier},
or the string ``\verb|#`|'' followed by an \syntacc{object-identifier}.
\end{itemize}
...
$C$ is head constructor and $s_1,\dots,s_n$ are sort parameters.
...
\label{grammar:patterns}
	...
	`(' <pattern> `,' <pattern> `)'
	\alt `\\mem' `{' <object-sort> `,'  <object-sort> `}'
	`(' <variable> `,' <pattern> `)'
	\alt `\\top' `{' <object-sort> `}' `(' `)'
...
A Kore definition contains a sequence of declarations.
Every declaration declares a sort, a symbol, an alias, or an axiom.
For now, a Kore definition contains just one flattened module that contains all 
declarations.
In the near future, Kore will be extended to support multiple modules, 
parametric modules, and module imports. 

\begin{grammar}\small
<definition> ::= <attribute> `module' <module-name> <declaration>$^*$ 
`endmodule' <attribute>
...
<sort-declaration> ::= `sort' `{' <sort-variable-list> `}' <object-sort> 
<attribute>
...
In sort and axiom declarations, the sort variable declarations follow by the 
keywords `\texttt{sort}' and `\texttt{axiom}'.
In symbol and alias declarations, the sort variable declarations follow by the 
head constructor being declared.
...
\item In any declaration, every sort variable used should be in the leading 
sort variable list;
...
\item Every sorts, symbols, and aliases should have unique names;
\end{enumerate}
...
\begin{center}
	\ttfamily
	sort\{$v_1,\dots,v_m$\} $C$\{$s_1,\dots,s_n$\} [$\att$]
\end{center}
where $v_1,\dots,v_m$ are sort variables,
\texttt{$C$\{$s_1,\dots,s_n$\}} is a sort which (possibly) uses sort 
variables $v_1,\dots,v_m$, and $\att$ is an attribute.
...
\denote{v} =
\begin{cases*}
\KVariableToKPattern\texttt{\{\}($\denote{v}_\var$)} & if $v$ is 
...
$\denote{\texttt{\slequals\{$s_1,s_2$\}($\varphi_1,\varphi_2$)}}
=
\texttt{\slequals\{$s_1,s_2$\}($\varphi_1,\varphi_2$)}$
& if $s_1,s_2$ are \syntacc{meta-sort}
\\
$\denote{\texttt{\slmem\{$s_1,s_2$\}($\varphi_1,\varphi_2$)}}
=
...
$\denote{\texttt{\slmem\{$s_1,s_2$\}($\varphi_1,\varphi_2$)}}
=
\texttt{\slmem\{$s_1,s_2$\}($\varphi_1,\varphi_2$)}$
& if $s_1,s_2$ are \syntacc{meta-sort}
\\
...
\begin{align*}
  &\denote{\_} \colon \syntacc{object-axiom-declaration} \to 
  ...
  if $\varphi$ is \syntacc{object-pattern} \\
  \texttt{\parametric{axiom}{$s'_1,\dots,s'_m$} $\denote{\varphi}$} & if
  $\varphi$ is \syntacc{meta-pattern}
  	\end{cases*}
\end{align*}
where $s_1',\dots,s_m'$ are all \syntacc{meta-sort-variable} in $s_1,\dots,s_n$.
...
develop tools about the Kore language.
The Kore language defined in Section~\ref{sec:kore} has a rather simple syntax.
...
As we do the first category of future work and define \K in Kore, we will 
...
Proof checkers should be very simple so that their correctness is convincing, 
and very fast so that they can handle proofs of monster size.
