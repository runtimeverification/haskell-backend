...
of potentially infinite sets of \emph{sorts} $S$, \emph{($S$-)symbols}
$\Sigma$, and \emph{($\Sigma$-)patterns} $A$, respectively, and provides models
that interpret the symbols relationally, which in turn yield a
\emph{semantic validity} relation $(S,\Sigma,A)\models\varphi$ between
theories $(S,\Sigma,A)$ and $\Sigma$-patterns $\varphi$.
Matching logic admits \emph{complete} deduction, denoted
$(S,\Sigma,A)\vdash\varphi$.
...
\todo{Now sorts and symbols take sort parameters. Easy fix. But we need to go 
through the rest and propagate this fix and make sure they make sense.}
