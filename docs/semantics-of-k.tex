...
\begin{figure}[hbtp]
	\begin{tabular}{ll}
		...
		From $\varphi$, derive $\forall x . \varphi$.
		\\
		\hline
		\propagationbottom &
		$\CSub{\sigma}[\bot] \imp \bot$
		\\
		\propagationvee &
		$\CSub{\sigma}[\varphi_1 \vee \varphi_2]
		\imp \CSub{\sigma}[\varphi_1] \vee \CSub{\sigma}[\varphi_2] $
		\\
		\prule{Propagation$_\exists$} &
		$\CSub{\sigma}[\exists x . \varphi]
		\imp \exists x . \CSub{\sigma}[\varphi]$
		\quad if $x \not\in \FV(\Csigmaapp{\exists x . \varphi})$
		\\
		\hline
		\framing &
		From $\varphi_1 \imp \varphi_2$, derive
		$\CSub{\sigma}[\varphi_1] \imp \CSub{\sigma}[\varphi_2]$
		\\
		\hline
		\existence &
		$\exists x . x$
