\documentclass[UTF8,11pt]{article}
  ...
  \newcommand{\Kdeduce}{\mathit{Kderivable}}
...
\section{Reflection in Matching Logic}

Consider a matching logic prover $P$ that decides whether a matching logic patter $\varphi$ holds in a matching logic theory $T$. 
...
Since any executable programs (in the normal sense) can only handle finite inputs, we are in particular interested in \emph{finite representations} of pattern and matching logic theories.

In Section~\cref{sec:finite-mechanisms} we have seen many mechanisms that help to define infinite theories using finite resources, including using sort, symbol, and pattern schemas with side conditions.
In this section, we will give such mechanisms \emph{a formal semantics} in terms of
\begin{itemize}
	\item developing a reflective theory of matching logic which we refer to as 
	the meta-theory $K$;
	\item axiomatizing the mechanisms that we use in 
	Section~\cref{sec:finite-mechanisms} to define infinite theories in a 
	finite manner in $K$.  
\end{itemize}

There has been a good many of studies about reflections in logic, computation, and programming.
Brian Smith gave reflection a rather broad definition in~\cite{?}, from which we quote that
\begin{displayquote}
...
subject matter.
\end{displayquote}
Following this definition, a reflective theory of matching logic is a matching logic theory with which one can study and discuss properties of matching logic, for example, the wellformedness of a pattern and the derivability of a pattern from a theory.
The following table shows the difference of topics that we are interested 
between a theory of arithmetics and its reflective theory:
\begin{center}
	\textbf{Topics we discuss in the theory of arithmetic}
	\begin{itemize}\itemsep0em
		\item Multiplication is commutative.
		\item Number $233$ is a prime number.
		\item The de-factorization of $12$ is $2 \times 2 \times 3$.
		\item The expression $\sqrt{-1}$ is not a well-defined number.
	\end{itemize}
	\textbf{Topics we discuss in the reflective theory of the theory of 
	arithmetic}
    \begin{itemize}\itemsep0em
    	\item The pattern $x \times y = y \times x$ is a derivable pattern.
    	\item The sort $\mathit{RealNumber}$ is declared and the sort 
    	$\mathit{ComplexNumber}$ is not declared.
    	\item The pattern $\mathit{isPrime(\varphi)}$ has sort 
    	$\mathit{Bool}$ 
    	if $\varphi$ has sort $\mathit{Nat}$.
    	\item The DNF of $\varphi \to \psi$ is $\varphi' \vee \psi'$, where 
    	$\varphi'$ and $\psi'$ are DNFs of $\neg \varphi$ and $\psi$, 
    	respectively.
    \end{itemize}
\end{center}

\paragraph{Naming relation} TODO.
...
In previous subsections we have defined a universe of terms that represent 
abstract syntax trees of matching logic sorts, symbols, and patterns.
Every sort or symbol or pattern gets its meta-representation in the meta-theory 
$\Kfinite$ as a term of sort $\KSort$ or $\KSymbol$ or $\KPattern$.

Start from this subsection, we are going to define matching logic theories and 
the proof system of matching logic as \emph{sets of predicates over the 
universe of abstract syntax trees of sorts, symbols, and patterns}.
The approach that we adopt in this proposal is a standard approach in the 
studies of reflective logics. 
For example in~\cite{?}, a unary predicate symbol 
$T$ called the \emph{truth predicate} is used in a reflective logic of 
propositional logic that satisfies the faithfulness conditions
\begin{center}
\begin{tabular}{c|c}
$
...
\end{tabular}
\end{center}
where $\widehat{\varphi}$ is the term that represents the formula $\varphi$ in 
the reflective logic.
Intuitively, the truth predicate $T$ captures the semantics of a formula being 
true.

In the following we will introduce and axiomatize a finite number of
predicate symbols in the 
meta-theory $\Kfinite$ that capture the semantics of the matching logic 
proof system, or more specifically, the semantics of
\begin{itemize}\itemsep0em
...
We point out here that there are multiple ways to design a reflective logic.
For example, we could develop the meta-theory in a way that not only sorts, 
symbols, and patterns have their abstract syntax trees as terms in the 
meta-theory, but also signatures and theories.
In such reflections (which we refer to as \emph{deep embedding}, compared to 
\emph{shallow embedding} which is the approach we adopt in this proposal), the 
derivability relation is captured by a \emph{binary} predicate symbol that 
takes representations of a pattern and a theory as its two arguments.
The reason we adopt shallow embedding in this proposal instead of deep 
embedding is more of 
practical concerns.
...
\improvement{We should 
	mention here that all the subterms / subpatterns that appear in the axioms 
	are 
	assumed to be well-formed. Also, the theory $T$ itself must be well-formed, 
	in 
	the sense that all axioms in $T$ are well-formed. We omit those premises 
	when 
	writing the axioms because writing all of them down need a lot of space, 
	and 
	will make the axioms less readable.}
...
\todo{Change fonts and remove the ``dot'' after each formulas.}
\todo{Change notations}
...
\begin{equation*}
\Kdeduce(\overline{\varphi \to (\psi \to_s  \varphi)})
\end{equation*}
...
\begin{equation*}
\Kdeduce(\overline{\forall_s  v . \varphi \to_{s_2} 
\varphi[y \cln s_1 / x \cln s_1]}).
\end{equation*}
...
\to \Kdeduce(\overline{\forall_{s_1}^{s_2} x . (\varphi \to_{s_2} 
\psi) \to_{s_2} (\varphi \to_{s_2} \forall_{s_1}^{s_2} 
x . \psi)}).
\end{equation*}
...
(\psi[\varphi_1/x \cln s] \to_{s_2} \psi[\varphi_2/x \cln s])}).
\end{equation*}

\paragraph{Rule (Df).}
$\vdash \ceil{x}$.
\begin{equation*}
\Kdeduce(\Kapplication(\Ksymbol(\quot{ceil}, \KconsKSortList(s_1, 
\KnilKPatternList), s_2), \overline{x \cln s_1})).
\end{equation*}

\paragraph{Rule (M1).}
$\vdash (x \in y) = (x = y)$.
\begin{equation*}
\Kdeduce(\overline{(x \in_{s_1}^{s_2} y) =_{s_2}^{s_3} (x =_{s_1}^{s_2} y)}).
\end{equation*}

\paragraph{Rule (M2).}
$\vdash x \in (\varphi \wedge \psi) = (x \in \varphi) \wedge (x \in \psi).$
\begin{equation*}
\Kdeduce(\overline{x \in_{s_1}^{s_2} (\varphi \wedge_{s_1} \psi) =_{s_2}^{s_3} 
(x \in_{s_1}^{s_2} \varphi) \wedge_{s_2} (x \in_{s_1}^{s_2} \psi)}).
\end{equation*}

\paragraph{Rule (M3).}
$\vdash x \in \neg \varphi = \neg (x \in \varphi)$.
\begin{equation*}
\Kdeduce(\overline{x \in_{s_1}^{s_2} \neg_{s_1} \varphi =_{s_2}^{s_3} 
...
\begin{equation*}
(\overline{x \cln s_1} \neq \overline{y \cln s_2})  \to \Kdeduce(\overline{(x 
\in_{s_1}^{s_3} \forall_{s_2}^{s_1} y . \varphi) =_{s_3}^{s_4} 
(\forall_{s_2}^{s_3} y . (x \in_{s_1}^{s_3} \varphi))}).
\end{equation*}
...
& (\overline{x \cln s_1} \neq \overline{y \cln s_2}) 
  \wedge \neg \KinKPatternList(\overline{y \cln s_2}, 
  \KgetFvs(\overline{\sigma(L, \varphi_i, R)}))
\\
\to & \Kdeduce(\overline{x \cln s_1 \in_{s_1}^{s_3} \sigma(L, 
...
\begin{equation*}
\Kdeduce(\varphi) \to \Kdeduce(\overline{\forall_{s_1}^{s_2} x . \varphi}).
\end{equation*}

\paragraph{Rule (Membership Introduction).}
If $\vdash \varphi$ and $x$ does not occur free in $\varphi$, then $\vdash x \in \varphi$.
\begin{equation*}
\Kdeduce(\varphi) \wedge \neg \KinKPatternList(\overline{x \cln s_1}, 
\KgetFvs(\varphi)) \to \Kdeduce(\overline{x \cln s_1 \in_{s_1}^{s_2} 
\varphi}).
\end{equation*}
...
\begin{equation*}
\Kdeduce(\overline{x \cln s_1 \in_{s_1}^{s_2} \varphi}) \wedge\neg 
\KinKPatternList(\overline{x \cln s_1}, \KgetFvs(\varphi)) \to \Kdeduce(
\varphi).
\end{equation*}

\subsection{Faithfulness: Finite Case}

\improvement{State the faithfulness theorem and why it is useful, and move the 
proof to appendix.}
...
\subsection{Parametric Sorts and Symbols Declarations as Predicates}
