...
theory $K = ( S_K, \Sigma_K, A_K )$ with $S_K$ a finite set  of sorts, 
...
\subsubsection{A Roadmap of The Definition of The Meta-Theory}
...
26 + 10 + 3 = 65$ functional constructors (more can be added is needed or desired):
\begin{center}
	\begin{tabular}{c c c}
...
\small
	\begin{longtable}{l|l}
	    ...
	    \KVariableToKPattern & Injection function from {\KVariable} to 
	    {\KPattern} \\
	    \KvariableAsPattern & Construct variable patterns of the form $x \cln 
	    s$ \\
	    ...
	    \parametric{\KsortDeclared}{$\shs$} & Check whether a sort is declared 
	    in the current theory 
	    \\
	    \parametric{\KsortsDeclared}{$\shs$}  & Check whether a list of sorts 
	    are declared in the current theory 
	    \\
	    \parametric{\KsymbolDeclared}{$\shs$} & Check whether a symbol is 
...
parameters in defining theory $T_L$, because strictly speaking meta-variables 
do not belong to the theory $T_L$, but rather a tool or a mechanism from the 
outside that help defining $T_L$.

There are of course more than one naming of $T_L$.
...
\underbrace{\parametric{\KsortDeclared}{\shs}(s \cln \KSort)}_\text{if $s$ is 
defined} \to 
\underbrace{\parametric{\KsortDeclared}{\shs}(\KqList(s \cln 
\KSort))}_\text{then 
...
\begin{align*}
 & \forall s' . (\underbrace{\parametric{\KsortDeclared}{\shs}(s')}_\text{if 
 $s'$ is defined in $T_L$} \to 
   \underbrace{\parametric{\KaxiomDeclared}{\shs}(\Psi_1)}_\text{then $\Psi_1$ 
   is an axiom in $T_L$}) \\
 & \forall s \forall s' . (\underbrace{\parametric{\KsortDeclared}{\shs}(s) 
 \wedge 
 \parametric{\KsortDeclared}{\shs}(s')}_\text{if $s, s'$ are defined in $T_L$} 
 \to 
...
the provability relation of finite theories (`` $\vdashfin$ ''), the one that 
...
\improvement{Many sentences are from the specification of SMT-LIB language 
(version 2.6), which I use very often as the main reference when writing this 
section. We need to rephrase those sentences when we make Kore public.}
...
The Kore language syntax is defined by BNF-style production rules, which we 
summarize in Section~\ref{kore-syntax-summary}.
The language accepted by there rules is a \emph{superset} of the Kore language 
as a legal Kore language source file should satisfy additional constraints such 
as wellformedness.

\subsubsection{Lexicon}

The syntax rules defined in this section are given directly with respect to 
streams of lexical tokens from the set defined in this subsection.

A Kore language source file, also called as a Kore definition or simply a 
definition, consists of a sequence of ASCII characters.
All lexical tokens defined below are limited to ASCII printable characters.

Kore supports C-style \emph{comments}.
...
An \syntacc{identifier} is either of the following three cases:
\begin{itemize}
\item An \syntacc{object-identifier}, which is a nonempty sequence of alphabets 
and digits and the character ``\verb|'|'' that starts with an alphabet and is 
not a \syntacc{keyword}
\item A \syntacc{meta-identifier}, which is the character ``\verb|#|'' followed 
by an \syntacc{object-identifier};
%\item A \syntacc{reserved-identifier}, which is the character ``\verb|\|'' 
...
\paragraph{Keyword.}
Kore has three keywords.
They are used to initiate declarations.
\begin{center}
	{\ttfamily
		\begin{tabular}{l|l}
...
\subsubsection{Sorts}
A sort in Kore is a \emph{sort variable}, an \emph{atomic sort}, or a 
\emph{sort constructor} applied to a list of sorts.
There is no syntactic difference among a sort variable, an atomic sort, or a 
sort constructor.
They are all identifiers.
However, we do want to distinguish meta-level sorts from object-level ones, by 
using \syntacc{meta-identifier} for meta-level sorts and 
\syntacc{object-identifier} for object-level sorts.
...
\alt <object-atomic-sort>
\alt <object-sort-constructor> `{' 
<object-sort-list> `}'

<object-sort-variable> ::= <object-identifier>

<object-atomic-sort> ::= <object-identifier>

<object-sort-constructor> ::= <object-identifier>

<object-sort-list> ::= <object-sort> | <object-sort> `,' <object-sort-list>

<meta-sort> ::= \quad
\alt <meta-sort-variable>
\alt <meta-atomic-sort>

<meta-sort-variable> ::= <meta-identifier>

<meta-atomic-sort> ::= `#Char' | `#CharList' | `#String' | `#Sort' | `#SortList'
...
Heads are user-definable things (a better word?) that are used to construct 
\syntacc{pattern} in prefix form (see Section~\ref{sec:kore-syntax-patterns}).
Heads include symbols and aliases.
Heads \emph{do not} include logic connectives such as ``\verb|\and|'' and 
``\verb|\or|''.
The difference is a head is \emph{either object-level or meta-level},
while logic connectives can be used in \emph{both object-level and meta-level}. 

\begin{grammar}\small
	<head> ::= <object-head> | <meta-head>
	
	<object-head> ::= <object-constant-head> | <object-nonconstant-head>
	
	<object-constant-head> ::= \quad
	\alt <object-atomic-constant-head>
	\alt <object-constant-head-constructor> `{' <object-sort-list> `}'
		
	<object-atomic-constant-head> ::= <object-identifier>

	<object-constant-head-constructor> ::= <object-identifier>
	
	<object-nonconstant-head> ::= \quad
	\alt <object-atomic-nonconstant-head>
	\alt <object-nonconstant-head-constructor> `{' <object-sort-list> `}'
	
	<object-atomic-nonconstant-head> ::= <object-identifier>
	
	<object-nonconstant-head-constructor> ::= <object-identifier>
	
	<meta-head> ::= <meta-constant-head> | <meta-nonconstant-head>
	
	<meta-constant-head> ::= \quad
	\alt <meta-atomic-constant-head>
	\alt <meta-constant-head-constructor> `{' <meta-sort-list> `}'
	
	<meta-nonconstant-head> ::= \quad
	\alt <meta-atomic-nonconstant-head>
	\alt <meta-nonconstant-head-constructor> `{' <meta-sort-list> `}'
	
	<meta-atomic-constant-head> ::= <meta-identifier>
	
	<meta-constant-head-constructor> ::= <meta-identifier>
	
	<meta-atomic-nonconstant-head> ::= <meta-identifier>
	
	<meta-nonconstant-head-constructor> ::= <meta-identifier>
\end{grammar}
...
\begin{grammar}\small
	...
	<object-pattern> ::= \quad
    \alt <object-variable>
    \alt <object-constant-head>
	\alt <object-nonconstant-head> `(' <pattern-list> `)'
	\alt `\\and' `{' <object-sort> `}'
	`(' <pattern> `,' <pattern> `)'
	\alt `\\not' `{' <object-sort> `}'
	`(' <pattern> `)'
	...
	`(' <pattern> `,' <pattern> `)'
	\alt `\\ceil' `{' <object-sort> `}'
	`(' <pattern> `)'
	\alt `\\floor' `{' <object-sort> `}'
	`(' <pattern> `)'
	\alt `\\equals' `{' <object-sort> `}'
	`(' <pattern> `,' <pattern> `)'
	\alt `\\mem' `{' <object-sort> `}'
	`(' <pattern> `,' <pattern> `)'
	\alt `\\top' `{' <object-sort> `}'
	...
	<meta-pattern> ::= \quad
    \alt <meta-variable>
    \alt <meta-constant-head>
	\alt <meta-nonconstant-head> `(' <pattern-list> `)'
	\alt `\\and' `{' <meta-sort> `}'
	`(' <pattern> `,' <pattern> `)'
	\alt `\\not' `{' <meta-sort> `}'
	`(' <pattern> `)'
...
\subsubsection{Declarations and Definitions}
...
<declaration> ::= \quad
\alt `sort' <object-sort>
\alt `symbol' <object-constant-head> `:' <object-sort>
\alt `symbol' <object-nonconstant-head> `(' <object-sort-list> `)' `:' 
<object-sort>
\alt `symbol' <meta-constant-head> `:' <meta-sort>
\alt `symbol' <meta-nonconstant-head> `(' <meta-sort-list> `)' `:' 
<object-sort>
\alt `alias' <object-constant-head> `:' <object-sort>
\alt `alias' <object-nonconstant-head> `(' <object-sort-list> `)' `:' 
<object-sort>
\alt `alias' <meta-constant-head> `:' <meta-sort>
\alt `alias' <meta-nonconstant-head> `(' <meta-sort-list> `)' `:' 
<object-sort>
\alt `axiom'  <pattern>
\alt `axiom' `{' <sort-variable-list> `}' <pattern>
