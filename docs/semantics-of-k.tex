...
\subsection{Ignore Me: Kore Language for Finite Theories}
The Kore language that we propose here is the finite and minimal one, in the 
sense that
\begin{itemize}
	\item Every Kore definition defines one finite matching logic theory, by 
	defining its (finite number of) sorts, symbols, and axioms;
	\item It supports no module-level operations such as module importation, 
	signature hiding, symbol renaming;
	\item It supports only the five basic matching logic constructors 
...
\begin{grammar}\small
	...
	<declaration>   ::= <sort-declaration> | <symbol-declaration> | 
	<axiom-declaration>
	
	<sort-declaration> ::= 	`sort' <sort> | `hooked-sort' <sort>
	
	<symbol-declaration> ::= `symbol' <symbol> `(' <sortlist> `)' `:' <sort>
	\alt `hooked-symbol' <symbol> `(' <sortlist> `)' `:' <sort>
	
	<axiom-declaration> ::= `axiom' <pattern>
	
	<sortlist>      ::= `' | <sort> | <sort> `,' <sortlist>
	
	<pattern> ::= <variable-name> `:' <sort>
	\alt <symbol> `(' <patternlist> `)'
...
\begin{grammar}\small
	...
	<symbol-expression> ::= \quad
	\alt <symbol-expression> `(' <sort-list> `)' `:' <sort>
	
	<symbol-expression> ::= <nonparametric-symbol>
	| <symbol-constructor> `{' <sort-variable-list> `}'
	
	<symbol-name> ::= "[a-zA-Z0-9]"$^*$
