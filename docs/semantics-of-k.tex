...
\begin{notation}
	...
	Subscripts are used ($\denote{\_}_0$, $\denote{\_}_1$, \dots) when we need 
...
Logic connectives constructor are lifted to non-parametric meta symbols by 
appending a ``\texttt{\sharpsymbol}'' in front.
\subsubsection{Lift Object Sorts}
Object sort constructors are lifted to non-parametric meta symbols constructors 
by appending 
a ``\texttt{\sharpsymbol}'' in front.
A sort variable $\tt S$ is lifted to a variable pattern {\tt \sharpsymbol 
$\tt S\cln \KSort$}.
A sort $\parametric{C}{s_1,\dots,s_n}$ with sort constructor $C$ and a list of 
sort parameters is lifted as
$$ \denote{\parametric{C}{s_1,\dots,s_n}} = 
\parametric{\denote{C}}{}(\denote{s_1}, \dots,\denote{s_n})$$

For example,
\begin{center}{\ttfamily
		\begin{tabular}{llr}
			\textrm{Object Sort} & \textrm{Lifting To} &  \textrm{Meta-Pattern}
			\\\hline
...
\end{center}
\subsubsection{Lift Symbol Declarations}
