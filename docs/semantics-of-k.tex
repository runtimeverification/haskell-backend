...
For example, one could use a formal system like Coq \cite{coq},
Isabelle~\cite{isabelle}, Agda~\cite{agda}, Vampire~\cite{vampire},
Maude~\ref{maude}, etc., and encode all matching logic elements in it:
...
change/improvement.
Therefore, we take a different approach here, which is also common:
\emph{we define the formal semantics of matching logic in (a fragment of)
matching logic itself}.
Specifically, in the \emph{finite} fragment of matching logic, which is
much simpler and is well-understood.
This not only incentivizes an accelerated development of implementations
of matching logic, such as \K, but also serves as a basis for simpler
and cleaner implementations of other backends for \K, because it is much
simpler and cleaner to encode finite theories than infinite ones.
...
We define the meta-theory $K$ as \emph{a syntactic theory},
...
$$ \Kqcons \colon \underbrace{\KSort}_\text{the sort parameter} \times 
