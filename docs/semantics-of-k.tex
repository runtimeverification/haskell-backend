...
in Section~\cref{sec:meta-theory}, a.k.a. the faithfulness theorem.
\begin{center}
	\begin{tabular}{cc}
		$
		\prftree[r]{\footnotesize (Upward Reflection)}
...
language (Section~\cref{sec:kore-semantics}).
...
In this section, we will complete define lexicon tokens and syntactic 
categories of the 
Kore language.

\subsubsection{Kore Comments}
Kore supports C-style comments.
Line comments start with ``\verb|//|'' and block comments start with 
``\verb|/*|'' and end with ``\verb|*/|''.

\subsubsection{Lexicon Tokens}
...
symbols, as we will se in Section~\cref{sec:kore-syntax-pattern}.

\subsubsection{Sort}
A sort is either a sort variable, or of the form 
$\parametric{C_\itsort}{s_1,\dots,s_n}$ for some $n \ge 0$, in which 
$C_\itsort$ is called a \emph{sort constructor} and 
