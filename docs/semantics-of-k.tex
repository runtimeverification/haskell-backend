...
\begin{Verbatim}[fontsize=\small,numbers=left,xleftmargin=5mm]
     ...
     *  - merge    is partial
     *  - merge    is assoc-comm
     *  - emptyMap, bind, and merge modulo AC are constructors
    ...
    sort   Ctxt{S1, S2}
    symbol gamma0({S1}, {S2})            : Ctxt{S1, S2}
    symbol ctxtapp(Ctxt{S1, S2}, {S1})   : {S2}
    
    /* Define alias gamma(HOLE:S1, BODY, Ctxt{S1, S2}) */
    alias  gamma(_:_,_,_,_) := \exists(#1:#2,gamma0(#1:#2,#3),#4)
