...
In this section, we present a matching logic theory $\Kfinite = \langle S_\Kfinite, \Sigma_\Kfinite, F_\Kfinite \rangle$ as \emph{the theory of finite matching logic theories}, in which $S_\Kfinite$ , $\Sigma_\Kfinite$, and $F_\Kfinite$ are finite sets of sorts, symbols, and axioms, respectively.
...
For that reason, we introduce the \emph{lifting and dropping morphisms}.
\unsure{I am truly not sure what the right word is here. Morphisms? Mappings? Operators? I don't know, but I am pretty it can't be Functions, because its domain is the set of all theories, which is not a set (or is it?).}

\newcommand{\lift}{\mathit{lift}}
\newcommand{\liftsort}{\mathit{lift_{sort}}}
\newcommand{\liftsymbol}{\mathit{lift_{symbol}}}
\newcommand{\liftpattern}{\mathit{lift_{pattern}}}
\newcommand{\liftsignature}{\mathit{lift_{signature}}}
\newcommand{\lifttheory}{\mathit{lift_{theory}}}


\begin{definition}[Lifting morphism]
	The lifting morphism \todo{WIP now.}
	$$\lift = \{\liftsort, \liftsymbol, \liftpattern, \liftsignature, \lifttheory\}$$ 
	is a family of five morphisms that have the following properties.
	
	Definition~(Sort-lifting morphism $\liftsort$).
	The sort-lifting morphism $\liftsort$ is a morphism from the collection of all finite set of sorts to the collection of all functions from finite sets of sorts to $\PATTERNS_\KSort$, i.e. the set of all $\KSort$-patterns. 
	For any finite set of sorts $S$, 
	$$\liftsort(S) \colon S \to \PATTERNS_\KSort$$
	is an injection function:
	$$\liftsort(S)(s_1) \neq \liftsort(S)(s_2) \quad \text{for any distinct $s_1$ and $s_2$ in $S$}.$$
	The \emph{lifting of $s$} where $s$ is a sort in $S$, denoted as $\hat{s}$, is defined as
	$$\hat{s} = \liftsort(S)(s).$$
	The \emph{lifting of $S$}, denoted as $\hat{S}$, is defined as
	$$\hat{S} = \{\hat{s} \in \PATTERNS_\KSort \mid s \in S \}.$$
	
	Definition~(Symbol-lifting morphism $\liftsymbol$).
	For any finite symbol set $\Sigma$,
	$$\liftsymbol(\Sigma) \colon \Sigma \to \PATTERNS_\KSymbol$$
	is an injection function, such that for any symbol $\sigma \in \Sigma_{s_1 \dots s_n,s}$,
	$$\liftsymbol(\Sigma)(\sigma) = \Ksymbol(\dots, )
	$$
TODO:: finish the definition of lifting.
\end{definition}

\begin{lemma}
	Suppose $T = \langle S, \Sigma, F \rangle$ is a finite matching logic theory and $\varphi$ is a $\Sigma$-pattern.
	The following proposition holds:
	\begin{itemize}
		\item If $\varphi$ is a logical axiom, then $K \vdash \Kdeduce(\hat{T}, \hat{\varphi});$
		\item If $\varphi \in F$, then $K \vdash \Kdeduce(\hat{T}, \hat{\varphi});$
		\item If
...
\begin{proof}[Proof Sketch of Theorem~\ref{thm:faithfulness-finite}]\quad
	...
	Step 1.1. Suppose the proof of $F \vdash \varphi$ is a sequence $\varphi_1 , \dots,  \varphi_n$ with $\varphi_n = \varphi$. By definition, for each $i \in \{1,\dots, n\}$, one of the following holds:
	\begin{itemize}
    ...
    Step 1.2. By Lemma~\ref{?}, for each $i \in \{1, \dots, n\}$, one of the following holds:
    \begin{itemize}
    	\item $K \vdash \Kdeduce(\hat{T}, \hat{\varphi_i})$;
    	\item $K \vdash \Kdeduce(\hat{T}, \hat{\varphi_i})$;
    \end{itemize}
	...
	Step 2.4. By hypothesis, $M_T \vDash \Kdeduce(\hat{T}, \hat{\varphi})$.
