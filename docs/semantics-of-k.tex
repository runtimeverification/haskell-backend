...
\Sigma_K, F_K )$ as \emph{the meta-theory of matching logic}.
$K$ is a finite theory and has $S_K$ , $\Sigma_K$, and $F_K$ as 
its finite sets of sorts, symbols, and axioms, respectively.
$K$ also serves as a foundation of the Kore language, a specification language 
for matching logic theories that is introduced in Section~\ref{sec:kore}.
...
26 + 10 + 3 = 65$ functional symbols as constructors:
\begin{center}
	\begin{tabular}{c c c}
...
In Section~\ref{sec:finite-lists}, we will define a sort $\KCharList$ for 
finites lists of characters, and two functional constructors
\begin{align*}
& \KnilKCharList \colon \to \KCharList \\
& \KconsKCharList \colon \KChar \times \KCharList \to \KCharList
\end{align*}
that construct cons-lists of characters.

\begin{notation}
We write $\KString$ as an alias of $\KCharList$, and we write $\Kepsilon$ as 
an alias of $\KnilKCharList$ which represents the empty string.
\end{notation}
...
The only constructor is the functional symbol
$$
\Ksort \colon \KString \times \KSortList\footnote{The sort $\KSortList$ is 
	defined in Section~\ref{sec:finite-lists}.} \to \KSort
$$
which takes a string as the \emph{name} and a list of sorts as the 
\emph{parameters}. 

The sort $\KSymbol$ is the sort for matching logic symbols.
The only constructor is the functional symbol
\begin{equation*}
\Ksymbol \colon \KString \times \KSortList \times \KSortList \times \KSort \to 
\KSymbol
\end{equation*}
which in turn takes a string as the name, a list of sorts as parameters, a list 
of argument sorts, and a return sort.

Two useful getter functions are defined as follows
\begin{align*}
...
with axioms
\begin{align*}
 & \KgetArgumentSorts(\Ksymbol(f, S, S', s)) = S'\\
 & \KgetReturnSort(\Ksymbol(f, S, S', s)) = s
\end{align*}
where $f$ is a $\KString$ variable, $S,S'$ are $\KSortList$ variables, and $s$ 
is 
a $\KSort$ variable.
...
In the meta-theory $K$, a reflective symbol $\KSymbolceil$ is defined and 
axiomatized:
\begin{align*}
&  \KSymbolceil \colon \KSort \times \KSort \to \KSymbol \\
&  \KSymbolceil(s, s') = \Ksymbol(\qquottt{ceil}, (s,s'), (s), s')
\end{align*}
where (following Notation~\ref{notation:lists})
\begin{center}
\begin{tabular}{ll}
& $(s,s')$ is a shortcut of 
$\KconsKSortList(s,\KconsKSortList(s',\KnilKSortList))$ \\
and & $(s)$ is a shortcut of $\KconsKSortList(s,\KnilKSortList)$
\end{tabular}
\end{center}

\begin{remark}
	Symbol {\KSymbolceil} is the reflection of definedness symbols \emph{in the 
	object theories}, not the definedness symbols in the meta-theory $K$.
\end{remark}
...
$\KPattern$\footnote{The sorts $\KVariable$ and $\KPattern$ are defined in 
Section~\ref{sec:ML-patterns}.}
in the meta-theory,
we define $\KCharList$, $\KSortList$, $\KSymbolList$, $\KVariableList$, 
and $\KPatternList$ as the sorts for finite lists over them, respectively.
Formally, for any sort $\ttX$
$$
\ttX \in \{\ttChar, \ttSort, \ttSymbol, \ttVariable, \ttPattern\}
$$
the sort $\XList$ is defined with two functional constructors:
\begin{align*}
  & \KnilXList \colon \to \XList
  \\
  & \KconsXList \colon \texttt{\sharpsymbol}\ttX \times \XList \to \XList
\end{align*}

\begin{notation}
\label{notation:lists}
	To write list expressions more compactly, we use abbreviation
	\begin{equation*}
	  (x_1, x_2, \dots, x_n) \equiv \KconsXList(x_1, \KconsXList(x_2, \dots\KconsXList(x_n, \KnilXList)\dots))
	\end{equation*}
\end{notation}

Common list operations and predicates are defined as usual.
...
\end{align*}
The symbol schema $$\parametric{\KinXList}{\shs} \colon 
\texttt{\sharpsymbol}\ttX \times \XList \to \shs \quad \text{for any 
$\shs \in S_K$}$$
defines for each sort $\shs$ in the meta-theory $K$, a predicate symbol that
checks list membership and returns either $\top_{\smallshs}$ or 
$\bot_{\smallshs}$:
\begin{align*}
  & \neg \parametric{\KinXList}{\shs}(x, \KnilXList)
  \\
  & \parametric{\KinXList}{\shs}(x, \KconsXList(y, L)) = (x = y) \vee 
  \parametric{\KinXList}{\shs}(x, L)
\end{align*}
Notice that using symbol (and axiom) schemas does not break the finitary 
of $K$ as long as we do not use sort schemas.
Sort schemas inductively define an infinite set of sorts, and thus will result 
in an infinite theory.
While in $K$ the number of sorts are designed to be finite (see Section~\ref{} 
for a summary of all sorts, symbols, and axioms in $K$).

The functional symbol $$\KdeleteXList \colon \texttt{\sharpsymbol}\ttX \times 
\XList \to \XList$$
takes an element and a list, and returns the list in which all the occurrences
of the element have been deleted, and the order of the remaining elements does 
not change.
...
It has the following axiom schema for any sort $\shs \in S_K$
\begin{align*}
\neg(\parametric{\KinKPatternList}{\shs}(\Kvariable(\KfreshName(L), s), 
\KgetFvsFromPatterns(L)))
\end{align*}
...
This fact is captured in $K$ by the next axiom schema for any sort $\shs \in 
S_K$
$$
...
A theory declares some sorts, so in $K$ there is a predicate symbol schema
$$ \parametric{\KsortDeclared}{s} \colon \KSort \to s \quad \text{for any $s 
\in S_K$}$$
A theory declares some symbols, so in $K$ there is a predicate symbol schema
$$ \parametric{\KsymbolDeclared}{s} \colon \KSymbol \to s\quad \text{for any $s 
\in S_K$}$$
with an axiom that says the definedness symbol is always declared
$$ \parametric{\KsortDeclared}{s}(s_1) \wedge 
\parametric{\KsortDeclared}{s}(s_2) \to 
\parametric{\KsymbolDeclared}{s}(\KSymbolceil(s_1, s_2))$$
A theory declares some patterns as axioms, so in $K$ there is a 
predicate symbol schema
$$ \parametric{\KaxiomDeclared}{s} \colon \KPattern \to s\quad \text{for any $s 
\in S_K$}$$
...
The predicate symbol schema
$$\parametric{\KwellFormed}{s} \colon \KPattern \to s \quad \text{for any $s 
	\in S_K$}$$ 
decides whether a pattern is \emph{wellformed pattern}.
A related predicate symbol $\parametric{\KwellFormedPatterns}{s}$ checks 
whether multiple 
patterns are all wellformed
$$ \parametric{\KwellFormedPatterns}{s} \colon \KPatternList \to s \quad 
\text{for any $s \in S_K$}$$
with two axioms
\begin{align*}
& \parametric{\KwellFormedPatterns}{s}(\KnilKPatternList) \\
& \parametric{\KwellFormedPatterns}{s}(\varphi, L) = 
\parametric{\KwellFormed}{s}(\varphi) 
\wedge 
\parametric{\KwellFormedPatterns}{s}(L)
\end{align*}
...
system and the correspondent axioms of $\parametric{\Kdeduce}{\shs}$. We recall 
the readers our 
...
All axioms are parametric for any sort $\shs \in S_K$.

(\textsc{Axiom}).\quad
$A \vdash \varphi$ if $\varphi \in A$.
\begin{equation*}
\parametric{\KaxiomDeclared}{\shs}(\varphi) \to 
...
\parametric{\KwellFormed}{\shs}(\overline{\varphi \to_s (\psi \to_s  \varphi)}) 
\to
...
\begin{center}
	\begin{tabular}{l|l}
		...
		\hline
		\KChar & Characters \\
		{\KCharList} or \KString & Finite lists of characters \\
...
\subsubsection{Symbols}
	...
	\begin{tabular}{l|l}
	    ...
	    \hline
	    \quottt{a}, \quottt{b}, \dots & Individual characters \\
	    {\KnilKCharList} or \Kepsilon, \KconsKCharList & Construct character 
	    lists (a.k.a. strings) \\
	    ...
	    \KSymbolceil & Definedness symbol(s) \\
	    \KnilKSortList, \KconsKSortList & Construct sort lists \\
	    ...
	    \KgetFvs & Get free variables in a pattern \\
	    \KgetFvsFromPatterns & Get all free variables from a pattern list \\
...
\label{sec:reflect}
We have shown the readers the big picture of the meta-theory $K$ and its usage 
in Section~\ref{sec:meta-theory}.
...
\begin{align*}
  ...
  \\
  &\qquad\qquad\qquad\quad
  \texttt{$\denote{S_1,\dots,S_n}_\itdeclared$,}
  \\
  &\qquad\qquad\qquad\quad
  \texttt{$\denote{\parametric{C_\itsort}{S_1,\dots,S_n}}_\itdeclared$)}
\end{align*}
where $\denote{\_}_\llist$ and $\denote{\_}_\itdeclared$ are two auxiliary 
lifting operations defined as follows.
The sort list lifting operation $\denote{\_}_\llist$ takes a list 
$\alpha$ of sorts 
and lifts them to a $\parametric{\KSortList}{}$ pattern as follows (using Kore 
syntax):
\begin{align*}
...
\end{cases*}
\end{align*}
The sort declared lifting function $\denote{\_}_\itdeclared$ is depended on a 
meta-sort variable \texttt{\sharpsymbol S} and  
takes a list $\alpha$ of sorts and lifts them 
to a meta-pattern as follows:
\begin{align*}
\denote{\alpha}_\itdeclared = 
\begin{cases*}
\parametric{\sltop}{\texttt{\sharpsymbol S}} & if $\alpha$ is empty list \\
\parametric{\sland}{\texttt{\sharpsymbol 
S}}\texttt{(\parametric{\KsortDeclared}{}($\denote{s}$),$\denote{
\beta}_\itdeclared$)}
& if 
$\alpha 
= s, \beta$
...
& \qquad\qquad\qquad \texttt{
	\parametric{\Kapplication}{}($\denote{
		C_\itsymbol}_\itsymbol$,$\collapse(\texttt{\sharpsymbol 
...
\begin{align*}
  ...
  \texttt{\Ksymbol($\denote{C_\itsymbol}_\name$,$\denote{
  		...
  		s}$))}
\end{align*}
and $\collapse(\alpha)$ for a list $\alpha$ of Kore patterns is defined as
\begin{equation*}
\collapse(\alpha) =
\begin{cases*}
\parametric{\KnilKPatternList}{} & if $\alpha$ is empty list \\
\parametric{\KconsKPatternList}{}(\varphi, \collapse(\beta)) & if $\alpha$ is 
$\varphi, \beta$
\end{cases*}
\end{equation*}
