...
\begin{definition}[Encoding and Lift]
...
\begin{proof}[Proof Sketch of Theorem~\ref{thm:faithfulness-finite}]\quad
	...
	Step 1 (The ``$\Rightarrow$'' part). We prove this by mimicking the proof of $F \vdash \varphi$ in $\Kfinite$.
	
	Step 1.1. Suppose the proof of $F \vdash \varphi$ is a sequence $\varphi_1 , \dots,  \varphi_n$ with $\varphi_n = \varphi$. 
	Conduct mathematical induction on $n$.
	By definition, for each $i \in \{1,\dots, n\}$, one of the following holds:
	\begin{itemize}
		\item $\varphi_i$ is a logical axiom;
		\item $\varphi_i \in F$;
		\item There exist $i_1, i_2 < i$ such that $\varphi_{i_2} = \varphi_{i_1} \to \varphi_i$;
		\item There exists $i_1 < i$ and a variable $x$ such that $\varphi_i = \forall x . \varphi_{i_1}$;
		\item There exists $i_1 < i$ and a variable $x$ that does not occur free in $\varphi_{i_1}$ such that $\varphi_i = \forall x . (x \in \varphi_{i_1})$;
		\item There exists $i_1 < i$ and a variable $x$ that does not occur free in $\varphi_i$ such that $\varphi_{i_1} = \forall x . (x \in \varphi_i)$. 
	\end{itemize}
	
	Step 2 (The ``$\Leftarrow$'' part).
	
	Step 2.1. Build a canonical model $M_K$ of $\Kfinite$.
	
	Step 2.2. Fix a finite matching logic theory $T = (S, \Sigma, F)$, a $\Sigma$-pattern $\varphi$, and an encoding $e$.
	
	Step 2.3. By hypothesis, $M_K \vDash \Kdeduce(\hat{T}, \hat{\varphi})$.
	
	Step 2.4. Interpret $\Kdeduce(\hat{T}, \hat{\varphi})$ in $M_K$ and get $ \bracket{\hat{T}} \vDash \bracket{\hat{\varphi}}$.
	
	Step 2.5. Establish an isomorphism between $T$ and $\bracket{\hat{T}}$, and get $T \vDash \varphi$.
