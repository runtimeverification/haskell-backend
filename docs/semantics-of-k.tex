\documentclass[UTF8,11pt]{article}
  ...
  \newcommand{\KMapReduceKgetFvs}{\mathit{KgetFvsFromPatterns}}
  ...
  \newcommand{\KMapKgetSort}{\mathit{KgetSortsFromPatterns}}
...
Strings are seen as finite lists of characters, which we will define in\todo{I haven't got a chance to work on this, but I will.} Section~\cref{sec:finite-lists}.  , denoted as $\KString$, is defined as an alias of the sort $\KCharList$regarded as cons-lists of characters.
...
Formally, for any sort $X$ where\info{Why not add $\KChar$ to this set and thus get rid of $\KString$, or just use $\KString$ as an alias for $\KChar$-lists? Xiaohong: True. We could do that.}
$$
...
It has five functional constructors:\info{Any reason why $\Kexists$ does
not take a variable? Xiaohong: Because there isn't a sort $\mathit{KVariable}$.}
\begin{align*}
...
\begin{notation}
	...
	The following notation conventions are adopted to write $\KPattern$ patterns in a more compact way, by putting a bar upon their normal mixfix forms:\info{$\sigma$ is a $\Ksymbol(...)$ thing, isn't it? Xiaohong: Yes, it is. And $\sigma$ is a $\KSymbol$ variable. I will add that to the notation convention. See Notation 6. }
	\begin{align*}
...
\begin{notation}
	As a convention, we use $\Sigma$ for $\KSignature$ variables.\improvement{I found it confusing to use the same letter $\Sigma$ for both $\KSymbolList$ variables and $\KSignature$ variables. We should use $\Sigma$ only for $\KSymbolList$ variables, and find a new letter for $\KSignature$ variables.} 
...
The map of $\KgetSort$ is a partial function
$$ \KMapKgetSort \colon \KPatternList \times \KSignature \rightharpoonup \KSortList
$$
that takes a pattern lists and applies $\KgetSort$ on each element.
The symbols $\KwellFormed$, $\KgetSort$, and $\KMapKgetSort$ are axiomatized by the following axioms:
\begin{align*}
