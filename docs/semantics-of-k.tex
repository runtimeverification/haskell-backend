...
For any (not necessarily distinct) sorts $s_1,s_2\in S$, assume the following
\emph{definedness} symbol and corresponding pattern:
$$
\begin{array}{l@{\hspace*{7ex}}l}
\lceil\_\rceil_{s_1}^{s_2} \in \Sigma_{s_1,s_2}
& \mbox{\it // Definedness symbol} \\
\lceil x\!:\! s_1 \rceil_{s_1}^{s_2} \in F & \mbox{\it // Definedness pattern}
\end{array}
$$
If $S$ is infinite, then we have infinitely many definedness symbols and patterns
above.
It can be easily shown that if $\varphi \in \Pattern_{s_1}$ then
$\lceil\varphi\rceil_{s_1}^{s_2}$ is a predicate; specifically,
if $\rho : \Var \ra M$ is any valuation then
...
\end{array}
$$
The totality construct states that the enclosed pattern must be matched by
all values.
...
If the sort decorations cannot be inferred from context, then we assume
