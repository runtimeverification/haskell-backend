...
For example, in the above example, (a)~is often called as \emph{parametric 
overloading}; (b)~is called as \emph{subsort overloading} if naturals is seen 
as a subsort of rationals; (c)~is called as \emph{ad hoc overloading} or 
...
Our proposal is to make implicit overloading explicit, and we have seen one 
such example.
In Section~\ref{sec:finite-mechanisms}, we have shown how to deal with 
parametric overloading by explicit writing the sort parameters.
...
\end{tabular}
\end{center}
while in its internal parse them as
\begin{center}
	\begin{tabular}{rl}
...
we say two symbols $\sigma_1$ and $\sigma_2$ are \emph{subsort overloading}, if 
...
<sort-declaration> ::= `sort' `{' <sort-variable-list> `}' 
