...
\footnote{The sort $\KPattern$ is defined in Section~\cref{sec:matching-logic-patterns}.}
in $S_K$,
...
takes a target pattern $\varphi$, a ``find''-pattern $\overline{x \cln s}$, and a ``replace''-pattern $\psi$, and returns $\varphi[\psi / x \cln s]$.
...
\begin{notation}
	...
	We often abbreviate $\Ksignature(S, \Sigma)$ as $\langle S, \Sigma \rangle$. 
\end{notation}
