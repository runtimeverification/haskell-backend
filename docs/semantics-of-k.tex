\documentclass[UTF8,11pt]{article}
  ...
  \newcommand{\Kmembership}{\mathit{\shp membership}}
...
\end{array}
$$
So $x$ has been ``pulled'' out of its context and isolated.
Now other semantic rules or axioms can be applied to reduce $x$.
At any moment during the reduction, the axioms above can be applied
backwards and thus the new $x$ can be ``plugged'' back into its
context.
...
,
\ \textsf{state}(l \mapsto a, R_\textit{state})
)
\\
\textsf{cfg}
(
...
\ \textsf{env}(x \mapsto l, R_\textit{env})
,
\ \textsf{state}((\gamma\hole_3.\;\hole_3)[l \mapsto a], R_\textit{state})
)
\\
\textsf{cfg}
(
...
\ \textsf{env}(x \mapsto l, R_\textit{env})
,
\ \textsf{state}((\gamma\hole_3.\;(\hole_3,R_\textit{state}))[l \mapsto a])
)
\\
\textsf{cfg}
(
...
\ \textsf{env}(x \mapsto l, R_\textit{env})
,
\ (\gamma\hole_3.\;\textsf{state}(\hole_3,R_\textit{state}))[l \mapsto a]
)
\\
...
\ \textsf{env}(x \mapsto l, R_\textit{env})
,
\ \textsf{state}(\hole_3,R_\textit{state})))[l \mapsto a]
\\
...
\\
(\gamma\hole_3.\;
...
,
\ \textsf{state}(\hole_3,R_\textit{state})))[x \mapsto l])[l \mapsto a]
\\
(\gamma\hole_2\hole_3.\;
\textsf{cfg}
(
...
\ \textsf{env}(\hole_2, R_\textit{env})
,
\ \textsf{state}(\hole_3,R_\textit{state})))[x]
...
\ \textsf{env}(\hole_2, R_\textit{env})
,
\ \textsf{state}(\hole_3, R_\textit{state})
))
...
Section~\cref{sec:meta-theory-finite}, while before that, we would like to
...
Lifting will be defined in detail in Section~\cref{sec:kore-semantics}, but before that we would like to use two examples to illustrate the big picture of how things work and to define a set of terminologies that we will use later.

\subsection{Example A: Arithmetic \& Parametric Lists}

\improvement{These two examples are helpful but I am not sure when to present them. Putting them here before introducing $K$ makes us using lots of symbols ($\#and$, $\#symbol$, etc) that are not defined yet.}
...
$K$ is finite and has $S_K$ , $\Sigma_K$, and $F_K$ being finite sets of sorts, symbols, and axioms, respectively.
We are going to define them in detail in the following subsections.
...
\subsection{Encode Theories in $K$}
...
\begin{theorem}[Faithfulness Theorem]\label{thm:faithfulness-finite}
	$$T \vdash \varphi \quad \text{iff} \quad \Kfinite \cup \denote{T} \vdash 
	\Kdeduce(\hat{\varphi}).$$
\end{theorem}
...
\label{sec:kore-syntax}

\todo[inline]{This section should include (1) a formal BNF grammar of Kore (2) namespaces for variable names (3) sort parameters \& sort parameters at the meta-level (4) Kore keywords that are reserved for sorts and symbols at the meta-level.}
\todo[inline]{We should decide a syntax for alias declaration.}
