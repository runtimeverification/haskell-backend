...
for dir in $(ls -d test-*); do
    ...
    case "$name" in
        ...
        "simplify")
            ...
            ;;
        "log-simplify-json")
            SERVER="${KORE_RPC_BOOSTER} --log-format json --log-context \"booster|kore>function*|simplification*,success|failure|abort\" --log-context \"booster|kore>function*|simplification*,match,failure|abort\"  --simplification-log-file test-$name/simplify-log.txt" ./runDirectoryTest.sh test-$name $@
            ;;
        "foundry-bug-report")
