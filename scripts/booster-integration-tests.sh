...
for dir in $(ls -d test-*); do
    ...
    case "$name" in
        ...
        "substitutions" | "vacuous" | "pathological-add-module")
            ...
            SERVER=$KORE_RPC_BOOSTER ./runDirectoryTest.sh test-$name $@
            ;;
        "get-model" | "collectiontest" | "implies" | "implies2" | "implies-issue-3941" | "implies-smt" | "set-matching" | "remainder-predicates" | "use-path-condition-in-equations" | "issue3764-vacuous-branch" | "3934-smt" | "booster-implies-consequent-simplification")
            SERVER=$BOOSTER_DEV ./runDirectoryTest.sh test-$name $@
            SERVER=$KORE_RPC_BOOSTER ./runDirectoryTest.sh test-$name $@
            ;;
        "condition-filtering")
        ...
        "questionmark-vars" | "simplify-smt")
            ...
            SERVER=$KORE_RPC_DEV ./runDirectoryTest.sh test-$name $@
            ;;
        "compute-ceil" | "no-evaluator" | "non-linear-int-requires" | "get-model-subsorts" | "simplify" | "escalate-partial-match")
            SERVER=$BOOSTER_DEV ./runDirectoryTest.sh test-$name $@
            ;;
        "log-simplify-json")
