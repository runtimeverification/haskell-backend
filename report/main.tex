...
\begin{proof}
	...
	\vdash (P = \bot) \to (\floor*{P} = \neg \top)
	\end{equation}
	
	The proof of~\eqref{eq:prop:017:b} is trivial and we left it as an exercise. We now prove~\eqref{eq:prop:017:a}.
...
The proof of~\eqref{eq:predicatevee-02} needs more effort. We first show that $\vdash (Q \neq \top) = (\floor*{Q} = \bot)$. Since $\floor*{Q}$ is a predicate pattern, it suffices to show $\vdash (Q = \top) = (\floor*{Q} = \top)$.

It is trivial to show $\vdash (Q = \top) \to (\floor*{Q} = \top)$. We show the other direction $\vdash (\floor*{Q} = \top) \to (Q = \top)$ through the following backward reasoning.

have the backward reasoning as follows.
\begin{align*}
&\vdash Q \neq \top \to (P \vee \floor*{Q} = \top) \\
\text{if} \quad &\vdash
\end{align*}
