...
\begin{prop} \label{prop:predicateor}
For any predicate patterns $P_1$ and $P_2$, $\vdash P_1 \vee P_2$ iff $\vdash P_1$ or $\vdash P_2$. 
\end{prop}
\begin{proof}
($\Leftarrow$) is obtained by Proposition~\ref{prop:vee-introduction}. We now prove ($\Rightarrow$).
\end{proof}
...
Because $\vdash P \vee \floor*{Q}$ and by Proposition~\ref{prop:predicateor}, either $\vdash P$ or $\vdash \floor*{Q}$. That $\vdash P$ implies $\vdash P \vee Q$ is by Proposition~\ref{prop:vee-introduction}. That $\vdash \floor*{Q}$ implies $\vdash Q$ is by Proposition~\ref{prop:floor} and the Membership Elimination.
\end{proof}
