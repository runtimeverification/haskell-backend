...
----------------------------------------
----------------------------------------
-- helpers for llvm timing histogram data
groups :: Integer -> [Time] -> [Int]
groups incrMS xs = groups' 0 $ sort xs
  where
    incT = 1e-6 * fromIntegral incrMS
    groups' start xs
        | null xs = []
        | otherwise =
            let (current, rest) = break (> start + incT) xs
             in length current : groups' (start + incT) rest

llvmTime :: ByteString
llvmTime = "^.*Performed LLVM call in (.*)$"

getLLVMTimes :: [ByteString] -> [Time]
getLLVMTimes = mapMaybe get
  where
    get l = case l =~ llvmTime of
                ("", _all, "", [bsTime]) -> Just $ read $ BS.unpack bsTime
                (_::ByteString, _::ByteString, _::ByteString, _) -> Nothing

llvmTimes :: Integer -> FilePath -> IO [String]
llvmTimes inc path =
    BS.readFile path >>=
        pure
        . zipWith (printf "%.6f %10d") [0, fromIntegral inc * 1e-6 :: Double .. ]
        . groups inc
        . getLLVMTimes
        . BS.lines
