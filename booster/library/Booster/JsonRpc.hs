...
respond stateVar =
    \case
        ...
        RpcTypes.Execute req -> withModule req._module $ \(def, mLlvmLibrary, mSMTOptions) -> Booster.Log.withContext CExecute $ do
            ...
            case internalised of
                Left patternError -> do
                    void $ Booster.Log.withContext CInternalise $ logPatternError patternError
                    pure $
                        Left $
        ...
        RpcTypes.AddModule RpcTypes.AddModuleRequest{_module, nameAsId = nameAsId'} -> Booster.Log.withContext CAddModule $ runExceptT $ do
        ...
        RpcTypes.Simplify req -> withModule req._module $ \(def, mLlvmLibrary, mSMTOptions) -> Booster.Log.withContext CSimplify $ do
            start <- liftIO $ getTime Monotonic
            let internalised =
        ...
        RpcTypes.Implies req -> withModule req._module $ \(def, mLlvmLibrary, mSMTOptions) -> Booster.Log.withContext CImplies $ do
            -- internalise given constrained term
            let internalised =
