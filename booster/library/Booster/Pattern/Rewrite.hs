            ...
            getPrettyModifiers >>= \case
                ModifiersRep (_ :: FromModifiersT mods => Proxy mods) -> do
                    ...
                    let modifiedPatternSubst = newSubsitution `compose` pat.substitution
                        ruleAndPatternSubst = modifiedPatternSubst `compose` ruleSubstitution

                    let rewrittenTerm = substituteInTerm ruleAndPatternSubst rule.rhs
                        substitutedNewConstraints =
                            Set.fromList $
                                map
                                    (coerce . substituteInTerm ruleAndPatternSubst . coerce)
                                    newConstraints
                    let rewritten =
                            Pattern
                                rewrittenTerm
                    ...
                    withContext CtxSuccess $ do
                        case unclearRequiresAfterSmt of
                            [] ->
                                withPatternContext rewritten $ pure (rewritten, Predicate FalseBool, ruleAndPatternSubst, Nothing)
                            _ -> do
                                rulePredicate <- mkSimplifiedRulePredicate ruleAndPatternSubst
