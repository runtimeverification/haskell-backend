...
-}
module Booster.Pattern.Existential (matchExistential, instantiateExistentials) where
...
-}
instantiateExistentials :: Set Predicate -> Predicate -> Predicate
instantiateExistentials known target@(Predicate targetTerm) =
    let candidates = map (matchExistential target) (Set.toList known)
     ...
     in Predicate $ substituteInTerm combinedSubst targetTerm
