...
import Booster.Pattern.Match (MatchResult(..), FailReason (..), matchTerms, MatchType (Implies))
import Booster.Pattern.Pretty (ModifiersRep(..), FromModifiersT, pretty')
import Booster.Pattern.Util (freeVariables, substituteInTerm, substituteInPredicate, sortOfPattern)
import Booster.Prettyprinter (renderDefault)
import Booster.SMT.Interface qualified as SMT
import Booster.Syntax.Json (prettyPattern, addHeader)
import Booster.Syntax.Json.Externalise (externaliseSort, externaliseExistTerm, externaliseSubstitution)
import Booster.Syntax.Json.Internalise (PatternOrTopOrBottom(..), internalisePatternOrTopOrBottom, pattern DisallowAlias, pattern CheckSubsorts, logPatternError, patternErrorToRpcError, )
...
runImplies :: Booster.Log.LoggerMIO m => KoreDefinition -> Maybe Booster.LLVM.API -> Maybe SMT.SMTOptions -> Kore.Syntax.KoreJson -> Kore.Syntax.KoreJson -> m (Either ErrorObj (RpcTypes.API 'RpcTypes.Res))
runImplies def mLlvmLibrary mSMTOptions antecedent consequent = getPrettyModifiers >>= \case
