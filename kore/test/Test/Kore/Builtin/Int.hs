...
test_unifyIntEq =
  ...
  where
    ...
    worker a b = case unify a b of
        Nothing -> empty
        Just unifyData -> lift $ Int.unifyIntEq (termUnification Not.notSimplifier) Not.notSimplifier unifyData

    unify a b = Int.matchUnifyIntEq a b <|> Int.matchUnifyIntEq b a
