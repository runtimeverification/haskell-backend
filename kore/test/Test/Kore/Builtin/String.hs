...
test_unifyStringEq =
  ...
  where
    ...
    unifyStringEq term1 term2 =
      ...
      where
        mpair
            = case String.matchUnifyStringEq term1 term2 of
                Just eqTerm -> Just (term2, eqTerm)
                Nothing -> case String.matchUnifyStringEq term2 term1 of
                    Just eqTerm -> Just (term1, eqTerm)
                    Nothing -> Nothing
