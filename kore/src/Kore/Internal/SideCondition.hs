...
andCondition
    sideCondition
    (from @(Condition _) @(MultiAnd _) -> condition2) =
        let (condition2', Assumptions termReplacements predicateReplacements) =
                simplifyConjunctionWithAssumptions
                    (Assumptions oldReplacements HashMap.empty)
                    condition2
                    & extract
            predicateReplacementsAsTerms =
                mapKeysAndValues
            ...
            replacements =
                termReplacements
                    <> predicateReplacementsAsTerms
            assumedTrue = condition1 <> condition2'
        in
            SideCondition
                { assumedTrue
                , replacements
...
simplifyConjunctionByAssumption =
    simplifyConjunctionWithAssumptions
        (Assumptions HashMap.empty HashMap.empty)

-- TODO: we might need to go back to converting the predicates to terms and back here
simplifyConjunctionWithAssumptions ::
    forall variable.
    InternalVariable variable =>
    Assumptions variable ->
...
simplifyConjunctionWithAssumptions
    previousAssumptions
    (toList -> andPredicates)
  =
    (fmap . Bifunctor.first) MultiAnd.make $
        flip runStateT previousAssumptions $
