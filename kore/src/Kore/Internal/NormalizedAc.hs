...
data InternalAc (normalized :: Type -> Type -> Type) key child = InternalAc
    { builtinAcSort :: !Sort
...
instance
    Eq (normalized key child) =>
    Eq (InternalAc normalized key child)
    where
    internal1 == internal2 =
...
instance
    Ord (normalized key child) =>
    Ord (InternalAc normalized key child)
    where
    internal1 <= internal2 =
