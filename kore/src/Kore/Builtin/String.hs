...
evalSubstr = Builtin.functionEvaluator evalSubstr0
  where
    ...
    evalSubstr0 _ _ _ resultSort arguments =
...
evalLength = Builtin.functionEvaluator evalLength0
  where
    evalLength0 _ _ _ resultSort arguments =
        Builtin.getAttemptedAxiom $ do
            let _str =
                    case arguments of
                        [_str] -> _str
...
evalFind = Builtin.functionEvaluator evalFind0
  where
    ...
    evalFind0 _ _ _ resultSort arguments =
        Builtin.getAttemptedAxiom $ do
...
evalString2Base = Builtin.functionEvaluator evalString2Base0
  where
    evalString2Base0 _ _ _ resultSort arguments =
        Builtin.getAttemptedAxiom $ do
            let (_str, _base) =
                    case arguments of
...
evalString2Int = Builtin.functionEvaluator evalString2Int0
  where
    evalString2Int0 _ _ _ resultSort arguments =
        Builtin.getAttemptedAxiom $ do
            let _str =
                    case arguments of
                        [_str] -> _str
...
evalChr = Builtin.functionEvaluator evalChr0
  where
    evalChr0 _ _ _ resultSort arguments =
        Builtin.getAttemptedAxiom $ do
            let _n =
                    case arguments of
                        [_n] -> _n
...
evalOrd = Builtin.functionEvaluator evalOrd0
  where
    evalOrd0 _ _ _ resultSort arguments =
        Builtin.getAttemptedAxiom $ do
            let _str =
                    case arguments of
                        [_str] -> _str
