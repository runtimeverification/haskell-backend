...
patternMatch' sideCondition ((MatchItem pat subject boundVars boundSet) : rest) deferred predicate subst = do
  ...
  where
        ...
        let varName = variableName var
         ...
         in if not $ Set.disjoint freeVars boundSet
                then failMatch "bound variable would escape binder"
                    ...
                    Nothing -> do
                        defined <- isTermDefined var term
                        patternMatch' sideCondition rest deferred defined (Map.insert (variableName var) term subst)
    ...
    isTermDefined var term = do
        isGloballyDefined <- Simplifier.isAssumedDefined
        if isGloballyDefined || SideCondition.isDefined sideCondition term || isSetVariable var
