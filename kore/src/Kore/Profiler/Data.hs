...
instance (MonadProfiler m) => MonadProfiler (ReaderT thing m ) where
    profileDuration a action =
        ReaderT (profileDuration a . runReaderT action)
    {-# INLINE profileDuration #-}

instance MonadProfiler m => MonadProfiler (Strict.StateT s m) where
    profileDuration a action =
        Strict.StateT (profileDuration a . Strict.runStateT action)
    {-# INLINE profileDuration #-}

instance MonadProfiler m => MonadProfiler (MaybeT m) where
    profileDuration a action = MaybeT (profileDuration a (runMaybeT action))
    {-# INLINE profileDuration #-}

instance MonadProfiler m => MonadProfiler (IdentityT m) where
    profileDuration a action =
        IdentityT (profileDuration a (runIdentityT action))
    {-# INLINE profileDuration #-}

instance MonadProfiler m => MonadProfiler (ExceptT e m) where
    profileDuration a action = ExceptT (profileDuration a (runExceptT action))
    {-# INLINE profileDuration #-}
