...
import Control.Exception(Exception, mask, mask_, catch, onException, bracketOnError)
import Control.Concurrent.Async (withAsync, asyncThreadId, AsyncCancelled(..))
...
data ReasonForHalting = HaltBranching {
                            branchingDepth :: !Int
                          } 
                      | HaltStuck {
                            stuckDepth :: !Int
...
srv  = do
  ...
  let sendResponses r = runStderrLoggingT $ runReaderT (sendBatchResponse r) qs
      ...
      spawnWorker = liftIO $ forkIO $ forever $
        bracketOnReqException
          (atomically $ readTChan reqQueue)
          cancelReq
          processReq

      mainLoop tid = receiveBatchRequest >>= \case
        Nothing -> do
          return ()
