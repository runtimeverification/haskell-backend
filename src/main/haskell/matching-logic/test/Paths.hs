...
--
-- The package root is the directory containing a `package.yaml` file. The
-- source tree used to build the test must remain unpacked. Any resources used
-- must be added to @extra-source-files@ in the package description.
        ...
        packageRoot = $(do
            ...
            let
                ...
                findPackageRoot here
                    | null here = fail ("Could not find package.yaml above " ++ bot)
                    | otherwise =
                        do
                            foundRoot <- TH.runIO $ doesFileExist (here </> "package.yaml")
                            if foundRoot
