...
import           Control.Exception (ErrorCall(ErrorCall), catch, evaluate)
...
variablesFreshIntCounterTests =
    testGroup
        ...
        , testCase
              ...
              (assertEqual
                   ...
                   (runIntCounter
                        ...
                        1))
        , testCase
              "Testing freshUnifiedVariable Meta 2."
              (assertEqual
                   ""
                   (metaVariable { variableName = Id "#var_2" }, 3)
                   (runIntCounter (freshVariable metaVariable) 2))
        , testCase
              "Testing failing freshVariableSuchThat Meta 1."
              ( (  evaluate
                  (runIntCounter
        ...
        , testCase
              "Testing freshVariableSuchThat Meta 1."
              (assertEqual
                   ""
                   (metaVariable { variableName = Id "#var_2" }, 3)
                   (runIntCounter
                        (freshVariableSuchThat metaVariable (const True))
                        2))
        ]
