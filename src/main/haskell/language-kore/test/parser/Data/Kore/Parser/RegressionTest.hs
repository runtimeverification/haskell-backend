module Data.Kore.Parser.RegressionTest
    ( InputFileName(..)
    , GoldenFileName(..)
    , regressionTest
    , regressionTests
...
regressionTests inputFiles =
    testGroup "Regression tests" (map regressionTestFromInputFile inputFiles)
...
goldenFromInputFileName (InputFileName inputFile) =
  ...
  where
    (directory, fileName) = splitFileName inputFile

toByteString :: Either String KoreDefinition -> LazyByteString.ByteString
toByteString (Left err) = LazyChar8.pack ("Parse error: " ++ err ++ ".")
...
runParser inputFileName verifyRequest = do
    ...
    let unverifiedDefinition = fromKore inputFileName fileContent
        definition =
