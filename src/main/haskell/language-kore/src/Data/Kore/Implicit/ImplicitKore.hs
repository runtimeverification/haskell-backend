...
getReturnSortAxiom = equalsAxiom (getReturnSortA [symbolA [vf, vS, vS', vs]]) vs
...
applicationP =
    symbol_ "#application" [symbolMetaSort, patternListMetaSort] patternMetaSort
...
mlPatternP DomainValuePatternType =
    symbol_ "#\\dv" [sortMetaSort, stringMetaSort] patternMetaSort
...
mlPatternP RewritesPatternType =
    symbol_
        ...
        [sortMetaSort, patternMetaSort, patternMetaSort]
        patternMetaSort
mlPatternP TopPatternType = symbol_ "#\\top" [sortMetaSort] patternMetaSort

[andA, bottomA, ceilA, _, equalsA, existsA, floorA, forallA, iffA, impliesA, inA, _, notA, orA, _, topA] =
    map mlPatternA allPatternTypes

[andP, bottomP, ceilP, _, equalsP, existsP, floorP, forallP, iffP, impliesP, inP, _, notP, orP, _, topP] =
    map mlPatternP allPatternTypes
...
variablePattern =
    symbol_ "#variablePattern" [stringMetaSort, sortMetaSort] patternMetaSort
...
patternAxioms =
    ...
    , equalsAxiom (impliesA [vs, vphi, vpsi]) (orA [vs, notA [vs, vphi], vpsi])
    , equalsAxiom
          (iffA [vs, vphi, vpsi])
    ...
    , equalsAxiom
          (topA [vs])
          (existsA
               [vs, variableA [vx, vs], variableAsPatternA [variableA [vx, vs]]])
    , equalsAxiom (bottomA [vs]) (notA [vs, topA [vs]])
    ]
...
getFVFromPatterns =
    symbol_ "#getFVFromPatterns" [patternListMetaSort] variableListMetaSort
...
getFVAxioms =
    ...
    , equalsAxiom (getFVA [applicationA [vsigma, vL]]) (getFVFromPatternsA [vL])
    , equalsAxiom
          (getFVA [andA [vs, vphi, vpsi]])
          (appendVariableListA [getFVA [vphi], getFVA [vpsi]])
    , equalsAxiom (getFVA [notA [vs, vphi]]) (getFVA [vphi])
    , equalsAxiom
          (getFVA [existsA [vs, v, vphi]])
          (deleteVariableListA [v, getFVA [vphi]])
    , equalsAxiom (getFVFromPatternsA [nilPatternListA]) nilVariableListA
    , equalsAxiom
...
occursFree =
    parameterizedSymbol_
        "#occursFree"
        [pS]
        [variableMetaSort, patternMetaSort]
        spS
...
substitutePatternAxioms =
    [ equalsAxiom
          ...
          (or_ (and_ (equalsS_ variableMetaSort vu v) vpsi)
               (and_
          ...
          (exists_
               ...
               (and_
                    (equals_
                         ...
                         (freshNameA
                              [ patternList_
                                    [vphi, vpsi, variableAsPatternA [v]]
                              ]))
                    (existsA
                         [ vs'
                         , variableA [vx', vs]
...
axiomDeclared = parameterizedSymbol_ "#axiomDeclared" [pS] [patternMetaSort] spS
...
wellFormedPatternsAxioms =
    [ parameterizedAxiom [pS] (wellFormedPatternsA [spS] [nilPatternListA])
    , parameterizedEqualsAxiom
          [pS]
...
getSortsFromPatterns =
    symbol_ "#getSortsFromPatterns" [patternListMetaSort] sortListMetaSort

getSortsFromPatternsA = applyS getSortsFromPatterns

getSortsFromPattersAxioms =
    [ equalsAxiom (getSortsFromPatternsA [nilPatternListA]) nilSortListA
    , equalsAxiom
...
wellFormedGetSortAxioms =
    ...
    , parameterizedEqualsAxiom
          ...
          (and_
               (and_ (wellFormedA [spS] [vphi]) (wellFormedA [spS] [vpsi]))
               (and_
    ...
    , parameterizedEqualsAxiom
          ...
          (and_ (wellFormedA [spS] [vphi]) (equals_ (getSortA [vpsi]) vs))
    , parameterizedEqualsAxiom
          [pS]
    ...
    , parameterizedEqualsAxiom
          ...
          (and_ (wellFormedA [sortMetaSort] [applicationA [vsigma, vL]]) vs)
    , parameterizedEqualsAxiom
          [pS]
          (getSortA [andA [vs, vphi, vpsi]])
          (and_ (wellFormedA [sortMetaSort] [andA [vs, vphi, vpsi]]) vs)
    , parameterizedEqualsAxiom
          [pS]
          (getSortA [notA [vs, vphi]])
          (and_ (wellFormedA [sortMetaSort] [notA [vs, vphi]]) vs)
    , parameterizedEqualsAxiom
          [pS]
          (getSortA [existsA [vs, v, vphi]])
          (and_ (wellFormedA [sortMetaSort] [existsA [vs, v, vphi]]) vs)
    ]
...
wellFormedImpliesProvableAxiom pattern1 =
    parameterizedAxiom
        [pS]
        (implies_ (wellFormedA [spS] [pattern1]) (provableA [spS] [pattern1]))
...
firstOrderLogicWithEqualityAxioms
 ...
 =
              ...
              impliesA
                  ...
                  , andA
                        ...
                        , existsA
                              ...
                              , vu
                              , equalsA
                                    [vs1, vs2, variableAsPatternA [vu], vpsi]
                              ]
                        , forallA [vs2, v, vphi]
                        ]
    ...
    , wellFormedImpliesProvableAxiom (equalsA [vs1, vs2, vphi, vphi])
...
membershipAxioms
 ...
 =
    [ let introductionFormula = inA [vs1, vs2, variableAsPatternA [v], vphi]
      in  parameterizedAxiom
              [pS]
              (implies_
                   (and_
...
axiomAxiom =
    parameterizedAxiom
        ...
        (implies_
             (wellFormedA [spS] [vphi])
             (implies_ (axiomDeclaredA [spS] [vphi]) (provableA [spS] [vphi])))
...
uncheckedKoreModule =
    Module
        ...
        , moduleSentences =
              ...
              ++ [asSentence getFV, asSentence getFVFromPatterns]
              ...
              ++ [asSentence getSort, asSentence getSortsFromPatterns]
              ...
              ++ [asSentence ceilBT, asSentence ceilBTAxiom]
