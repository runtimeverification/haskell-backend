...
fillCheckSort desiredSort (UnsortedPatternStub p) = Fix (p desiredSort)
...
fillCheckPairSorts
    :: MetaPatternStub
    -> MetaPatternStub
    -> Either (Sort Meta -> CommonMetaPattern, Sort Meta -> CommonMetaPattern) ( Sort Meta
                                                                               , CommonMetaPattern
                                                                               , CommonMetaPattern)
...
fillCheckPairSorts
    ...
    (SortedPatternStub SortedPattern { sortedPatternPattern = second
                                     , sortedPatternSort = s
                                     })
  =
    Right (s, Fix (first s), Fix second)
fillCheckPairSorts
    (SortedPatternStub SortedPattern { sortedPatternPattern = first
                                     , sortedPatternSort = s
                                     })
    (UnsortedPatternStub second)
  =
    Right (s, Fix first, Fix (second s))
fillCheckPairSorts
    (SortedPatternStub SortedPattern { sortedPatternPattern = p1
                                     , sortedPatternSort = actualSort1
                                     })
    (SortedPatternStub SortedPattern { sortedPatternPattern = p2
                                     , sortedPatternSort = actualSort2
                                     })
  =
    if actualSort1 /= actualSort2
        then error
            (  "Unmatched sorts: "
            ++ show actualSort1 ++ " and " ++ show actualSort2 ++ "."
            )
        else Right (actualSort1, Fix p1, Fix p2)
...
-}
unarySortedPattern
    :: (ResultSort -> ChildSort -> CommonMetaPattern -> PatternMetaType)
    -> Maybe ChildSort
    -> MetaPatternStub
    -> MetaPatternStub
unarySortedPattern constructor maybeSort patternStub =
  ...
  where
    (childSort, SortedPatternStub sortedPat) =
        case (maybeSort, patternStub) of
            (Just (ChildSort sort), _) -> (sort, withSort sort patternStub)
            (Nothing, SortedPatternStub sp) ->
                (sortedPatternSort sp, patternStub)
            (_, UnsortedPatternStub usp) ->
                error
...
-}
binaryPattern
    :: (Sort Meta -> CommonMetaPattern -> CommonMetaPattern -> PatternMetaType)
    -> MetaPatternStub
    -> MetaPatternStub
    -> MetaPatternStub
    ...
    case fillCheckPairSorts first second of
            ...
            case maybeSort of
                Nothing ->
                    error
                        ...
                        ++ " and " ++ show (secondPattern dummyMetaSort) ++ "."
                        )
        ...
        Right (commonSort, firstPattern, secondPattern) ->
            case maybeSort of
                ...
                Just (ChildSort s) ->
                    if s == commonSort
                        then patternFromChildSort
                            firstPattern
                            secondPattern
                            (ChildSort commonSort)
                        else error
                            (  "Incompatible sorts for equals children: "
                            ++ show s ++ " and " ++ show commonSort ++ "."
                            )
  where
...
equalsM_ s =
    binarySortedPattern
        (\(ResultSort resultSort) (ChildSort childSort) firstPattern secondPattern ->
...
ceilM_ s =
    unarySortedPattern
        (\(ResultSort resultSort) (ChildSort childSort) childPattern ->
             CeilPattern Ceil
