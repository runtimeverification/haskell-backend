...
liftSortListToMeta sortLifter =
    foldr (applyConsSortList . sortLifter) nilSortListMetaPattern
  where
...
instance LiftableToMetaML (Variable Object) where
    liftToMeta v =
        ...
        $ apply
            variableHead
            [liftToMeta (variableName v), liftToMeta (variableSort v)]
...
liftObjectReducer
    :: Pattern Object Variable CommonMetaPattern -> CommonMetaPattern
liftObjectReducer p =
    case p of
        AndPattern ap ->
            applyMetaMLPatternHead
                AndPatternType
                (liftToMeta (andSort ap) : getPatternChildren ap)
        ApplicationPattern ap ->
            let sa = applicationSymbolOrAlias ap
            in  Fix
                $ apply
                    (groundHead
                         (liftHeadConstructor
                              (getId (symbolOrAliasConstructor sa))))
        ...
        DomainValuePattern dvp ->
            applyMetaMLPatternHead
                ...
                [liftToMeta (domainValueSort dvp), domainValueChild dvp]
        EqualsPattern cp ->
            applyMetaMLPatternHead
                EqualsPatternType
        ...
        TopPattern bp ->
            applyMetaMLPatternHead TopPatternType [liftToMeta (topSort bp)]
        VariablePattern vp -> Fix $ apply variableAsPatternHead [liftToMeta vp]
  where
...
liftSortDeclaration ss = (symbolDeclaration, helperFunctionAxiom, declaredAxiom)
  where
    ...
    helperFunctionAxiom =
        SentenceAxiom
            ...
            , sentenceAxiomPattern =
                  SentenceMetaPattern . Fix . EqualsPattern
                  $ Equals
    ...
    declaredAxiom =
        SentenceAxiom
            ...
            , sentenceAxiomParameters = [sortParam]
            , sentenceAxiomPattern =
                  SentenceMetaPattern . Fix . ImpliesPattern
                  $ Implies
                      { impliesSort = SortVariableSort sortParam
                      , impliesFirst =
                            Fix
                            $ apply
                                (sortsDeclaredHead sortParamAsSort)
                                [liftToMeta sortParametersAsSorts]
                      , impliesSecond =
                            Fix
                            $ apply
                                (sortDeclaredHead sortParamAsSort)
            ...
            , sentenceAxiomPattern =
                  ...
                  $ Equals
                      ...
                      , equalsFirst =
                            Fix
                            $ apply
                                (groundHead liftedSymbolId)
                                (map liftToMeta sortParametersAsSorts ++ phis)
                      , equalsSecond =
                            Fix $ apply applicationHead [sigma, liftToMeta phis]
                      }
            }
    declaredAxiom =
        SentenceAxiom
            ...
            , sentenceAxiomPattern =
                  ...
                  $ Implies
                      ...
                      , impliesSecond =
                            Fix
                            $ apply (symbolDeclaredHead sortParamAsSort) [sigma]
                      }
            }

symbolOrAliasLiftedDeclaration
    :: SentenceSymbolOrAlias sa => sa Object pat variable -> MetaSentenceSymbol
  ...
  where
    ...
    symbolDeclaration =
        SentenceSymbol
            ...
            , sentenceSymbolSorts =
                  map (const sortMetaSort) sortParameters ++ patternSorts
    ...
    [ SentenceAliasSentence
          msa
              { sentenceAliasAttributes =
                   liftAttributes (sentenceAliasAttributes msa)
               }
    ]
...
liftSentence (ObjectSentence (SentenceSymbolSentence oss)) =
    let (mss, axiom1, axiom2) = liftSymbolDeclaration oss
    in  [ SentenceSymbolSentence mss
        , SentenceAxiomSentence axiom1
        , SentenceAxiomSentence axiom2
        ]
liftSentence (ObjectSentence (SentenceSortSentence ss)) =
    let (mss, axiom1, axiom2) = liftSortDeclaration ss
    in  [ SentenceSymbolSentence mss
        , SentenceAxiomSentence axiom1
        , SentenceAxiomSentence axiom2
        ]
liftSentence (MetaSentence (SentenceAxiomSentence as)) =
    [ SentenceAxiomSentence SentenceAxiom
          ...
          , sentenceAxiomAttributes =
                liftAttributes (sentenceAxiomAttributes as)
          , sentenceAxiomPattern =
                SentenceMetaPattern
