...
parseObjectId input =
    case Parser.parseOnly objectIdParser "<unlift>" input of
        ...
        _            -> Nothing
  where
    objectIdParser = idParser Object <* Parser.endOfInput
...
unliftSortConstructor
    SymbolOrAlias { symbolOrAliasConstructor = Id ('#':'`':name)
                  , symbolOrAliasParams = []
                  }
  =
    parseObjectId name
...
unliftHeadConstructor sa =
    case unliftSortConstructor sa of
...
instance UnliftableFromMetaML (Sort Object) where
    unliftFromMeta p =
        (SortVariableSort <$> unliftFromMeta p)
        ...
        | isImplicitHead variable (applicationSymbolOrAlias a) =
            case applicationChildren a of
                [uVariableName, uVariableSort] ->
                    pure Variable <*> unliftFromMeta uVariableName
...
unliftBinaryOpPattern unifiedCtor ctor [rSort, rFirst, rSecond] =
    unifiedCtor
    <$> (   pure ctor <*> unliftFromMeta (unliftResultOriginal rSort)
...
unliftDomainValuePattern [rSort, rChild] =
    DomainValuePattern
    <$> (   pure DomainValue <*> unliftFromMeta (unliftResultOriginal rSort)
    <*> pure (unliftResultFinal rChild)
    )
...
unliftCeilFloorPattern unifiedCtor ctor [rOperandSort, rResultSort, rChild] =
    unifiedCtor
    <$> (   pure ctor <*> unliftFromMeta (unliftResultOriginal rOperandSort)
...
unliftQuantifiedPattern
    ...
    -> (Sort Object
        ...
        -> p Object Variable UnifiedPattern)
    -> ([UnliftResult] -> Maybe (Pattern Object Variable UnifiedPattern))
unliftQuantifiedPattern unifiedCtor ctor [rSort, rVariable, rChild] =
    unifiedCtor
    <$> (   pure ctor <*> unliftFromMeta (unliftResultOriginal rSort)
