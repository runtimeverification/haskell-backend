...
pattern AtomOrListHead :: Text -> SExpr
pattern AtomOrListHead a <- (atomOrListHead -> Just a)

atomOrListHead :: SExpr -> Maybe Text
atomOrListHead (Atom x) = Just x
atomOrListHead (List (Atom x : _)) = Just x
atomOrListHead _ = Nothing
