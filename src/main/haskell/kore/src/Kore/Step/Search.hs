...
search
    ...
    -> (Limit Natural -> ExpandedPattern level variable
        -> Simplifier
            ( Tree
                ( Strategy prim
                , (ExpandedPattern level variable, StepProof level)
                )
             )
       )
    -> SearchConfiguration level variable
    -> Simplifier [UnificationSubstitution level variable]
search tools simplifier strategy config = do
    executionTree <- strategy stepLimit initialPattern
    let selectedConfigs = fst <$> pickStrategy executionTree
    matches <-
        catMaybes
        <$> traverse
            (runMaybeT . matchWith tools simplifier finalPattern)
            selectedConfigs
    case boundLimit of
        Unlimited -> return matches
        Limit n -> return (take (fromIntegral n) matches)
  where
...
matchWith tools simplifier e1 e2 = do
    ...
    (predSubst, _proof) <-
        give tools $ lift $ Predicate.evaluate simplifier predicate
