...
Description : Matches free-form patterns for Equals rules.
...
-- One must make sure that the two patterns have no common variables
-- (quantified or not).
  ...
  =
    case Ceil.makeEvaluateTerm tools second of
        (predicate, _proof) ->
            Just $ return
                PredicateSubstitution
                    -- TODO: The meaning of this is that the substitution holds
                    -- IF the predicate holds. Is an 'And' the proper thing to
                    -- use here? I guess so, because otherwise everything
                    -- becomes bottom.
                    { predicate = predicate
