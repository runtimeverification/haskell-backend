{
  ...
  outputs = { self, nixpkgs, haskell-nix, z3-src }:
    let
      ...
      haskell-src = {pkgs , lts ? null, postPatch ? ""} :
        pkgs.applyPatches {
          src = ./.;
          postPatch = ''
            ...
            ${if lts != null then ''sed -i 's/^resolver:.*$/resolver: ${lts}/g' stack.yaml'' else ""}
            ${postPatch}
          '';
        };
      ...
      projectOverlay = { pkgs, src, shell ? { }, compiler-nix-name ? "ghc8107" }:
        ...
        pkgs.haskell-nix.stackProject' ({
          ...
          self // {
          rematerialize-kore = pkgs.writeShellScriptBin "rematerialize-kore-nix-${compiler-nix-name}" ''
              #!/bin/sh
              ${self.stack-nix.passthru.generateMaterialized} ./nix/kore-${compiler-nix-name}.nix.d
            '';
        };

      projectForGhc = {ghcVersion, lts ? null}: perSystem (system:
        let
          pkgs = nixpkgsFor system;
          pkgs' = nixpkgsFor' system;
        in projectOverlay {
          inherit pkgs;
          src = haskell-src {inherit lts;  pkgs =pkgs'; postPatch = ''cp stack-nix-ghc9.yaml stack.yaml'';};
    ...
    in {
      ...
      project = projectForGhc {ghcVersion = "ghc8107";};
      projectGhc9 = projectForGhc {ghcVersion = "ghc902"; lts = "lts-19.9"; };
      ...
      packages = perSystem (system:
        self.flake.${system}.packages // {
          ...
          kore-exec-ghc9 = self.flakeGhc9.${system}.packages."kore:exe:kore-exec";

          # rematerializeGhc9 = rematerialize-kore {
          #   pkgs = nixpkgsFor' system;
          #   project = self.projectGhc9.${system};
          #   ghcVersion = "-ghc923";
          # };
        });
