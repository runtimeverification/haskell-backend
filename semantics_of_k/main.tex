...
\item No polymorphic symbols, so $\DEF$ will have $n^2$ definedness symbols if there are $n$ sorts;
\item And more \dots
\end{itemize}
\end{definition}
...
\comment{Given said that, there are cases when metavariables for variables make sense. In those cases we often want our metavariables to range over all variables of all sorts, in order to make our Kore definitions compact. The next example shows that.}
\end{remark}
...
\comment{There has been an argument on whether metavariables for patterns should be sorted or not. One important observation is that one may indeed want }
