...
Sort $\KBool$ is included in $S_K$. The following symbols of {\small 
BOOL} are included in $\Sigma_K$ with their usual axioms~\cite{?} added to 
$A_K$.
\begin{align*}
&\mathit{true}, \mathit{false} \ \colon \to \KBool \\
&\mathit{notBool} \ \colon \KBool \to \KBool \\
&\mathit{andBool}, \mathit{orBool} \ \colon \KBool \times 
\KBool \to \KBool\\
&\mathit{impliesBool} \ \colon \KBool \times \KBool \to 
\KBool.
\end{align*}

As a notation convention, we often write abbreviate the predicate pattern 
$\varphi = \mathit{true}$ as simply $\varphi$ for any pattern $\varphi$ of the 
sort $\KBool$.  Also we use quotation marks to write patterns of the sort 
$\mathit{String}$. For example, $\mathit{``one"}$ denotes the string ``one''.
...
&\varphi_e \in \varphi_l \quad \text{as a shorthand of $\mathit{inXList}(\varphi_e, \varphi_l) = \mathit{true}$}\\
&\varphi_e \not\in \varphi_l \quad \text{as a shorthand of $\mathit{inXList}(\varphi_e, \varphi_l) = \mathit{false}$}\\
...
The sort $\KSort \in S_K$ is the sort of matching logic sorts, whose only constructor symbol\footnote{A constructor symbol has its precise definition in matching logic. Please refer to~\cite{?}.} is $\mathit{sort} \colon \KString \to \KSort$.
The sort $\KSymbol \in S_K$ is the sort of matching logic symbols whose
only constructor symbol is $\mathit{symbol} \colon \KString \times \mathit{SortList} \times \KSort \to \KSymbol$. 
The sort $\KPattern \in S_K$ is the sort for ASTs of patterns, with the 
following functional constructor symbols:
\begin{align*}
&\Kvariable \colon \KString \times \KSort \to 
\KPattern \\
&\Kand, \Kor, \Kimplies, \Kiff \colon \KPattern \times \KPattern \to \KPattern \\
&\Knot \colon \KPattern \to \KPattern \\
&\Kapplication \colon \KSymbol \times \KPatternList \to 
\KPattern \\
&\Kexists, \Kforall \colon \KString \times 
\KSort \times \KPattern \to \KPattern \\
...
This abbreviation is seen as a shorthand, and all omitted sorts have to be explicitly written in ASTs, and that is the reason why $\Kequals$, $\Kcontains$, $top$, and $bottom$ take additional arguments of sort $\KSort$.
...
For example, the symbol $\mathit{wellFormed} \colon \KPattern \to \KBool$ 
determines whether a pattern is well-formed (or more precisely, it determines 
whether an abstract syntax tree is a well-formed one of a pattern.), with 
axioms:\todo{Provide axioms for all AST-related symbols.}
\begin{align*}
\mathit{wellFormed}(\Kvariable(x, s))
\end{align*}
The symbol $\mathit{getSort} \in \Sigma_{\KPattern, \KSort}$ takes a pattern and returns its sort. 
If the pattern is not well-formed, then $\mathit{getSort}$ returns $\bot_\KSort$; 
otherwise, $\mathit{getSort}$ returns $\mathit{sort}(s)$ if the pattern has sort $s$.
The symbol $\mathit{getFvs} \colon \KPattern \to \KPatternList$ collects all free variables in a pattern.
The symbol $\mathit{freshName} \colon \KPatternList \to \KString$ generates a deterministic variable name that does not occur free in patterns in the argument.
The symbol $\Ksubstitute \colon \KPattern \times \KPattern \times \KPattern \to \KPattern$ takes a target pattern $\varphi$, a ``find''-pattern $\psi_1$, and a ``replace''-pattern $\psi_2$, and returns $\varphi$ in which $\psi_2$ is substituted for $\psi_1$, denoted as $\varphi[\psi_2 / \psi_1]$.
All such AST-related symbols can be axiomatized in $K$. 
We take $\Ksubstitute$ as an example. 
...
&\quad = \Kexists(\mathit{freshName}(p, q, r), s,\\
&\qquad \quad \Ksubstitute((\Ksubstitute(p, \Kvariable(\mathit{freshName}(p, q, r), s), \\
&\qquad \quad \quad \Kvariable(x\cln\mathit{String}, s), q, r))\\
&\dots
\end{align*}

Side conditions can be defined as functional symbols from $\KPattern$ to 
$\KBool$. 
...
The sort $\mathit{Theory}$ is the sort of matching logic theories whose only constructor symbol is $\mathit{theory} \colon \mathit{Signature} \times \KPatternList \to \mathit{Theory}$, which takes a signature and an axiom set as arguments.
...
$\mathit{deducible} \colon \KPattern \to \KBool$, which returns $\mathit{true}$ 
...
\begin{align*}
\mathit{lift}[0]
&= \Kapplication(\mathit{symbol}(``\text{0}", \cdots),\dots) \\
&\neq \mathit{lift}[x - x] \\
&= \Kapplication(\mathit{symbol}(``\text{\_$-$\_}",\cdots), \dots)
\end{align*}
...
For example, the meta-variable $x \cln\cln s$ is lifted to a variable $x \cln \KPattern$ in $K$ with the constraint that $\mathit{getSort}(x \cln \KPattern) = sort(s)$. The function $\mathit{mvsc}[\_]$ collects all such meta-variable sort constraint in an object-pattern is implemented in Algorithm~\ref{alg:mvsc}.
...
\begin{algorithm}
	...
	\uElseIf{$\varphi$ is $x \cln \cln s$}{Return $x \cln \KPattern {\ \wedge\ } (\mathit{sort(s)} = \mathit{getSort}(x \cln \KPattern)) $}
	...
	{Return $\Kapplication(\mathit{symbol}(\sigma, (\mathit{sort}(s_1), \dots, \mathit{sort}(s_n)), \mathit{sort}(s)),$
...
\begin{algorithm}
	...
	{$C = C \cup (\mathit{sort(s)} = \mathit{getSort}(x \cln \KPattern))$}
	Return $\bigwedge C$\;
