...
Sorts $\mathit{Bool}$ and $\KString$ are included in $S_K$. The following symbols of {\small BOOL} are included in $\Sigma_K$ with their usual axioms~\cite{?} added to $A_K$.
\begin{align*}
...
As a notation convention, we use quotation marks to write patterns of the sort $\KString$. For example, $\mathit{``one"}$ denotes the string ``one''.
...
For example, the symbol $\mathit{wellFormed} \colon \KPattern \to \mathit{Bool}$ determines whether a pattern is well-formed (or more precisely, it determines whether an abstract syntax tree is a well-formed one of a pattern.), with axioms:\todo{Provide axioms for all AST-related symbols.}
\begin{align*}
...
Side conditions can be defined as functional symbols from $\KPattern$ to $\mathit{Bool}$. 
...
In $K$, the proof system of matching logic is captured by the functional symbol $\mathit{deducible} \colon \KPattern \to \mathit{Bool}$, which returns $\mathit{true}$ iff the argument pattern is a theorem. 
...
Therefore, the sorts $\mathit{Bool}$, $\KString$, $\KSymbol$, $\KSort$, and $\KPattern$ in the calculus $K$ are denoted as \verb|#Bool|, \verb|#String|, \verb|#Symbol|, \verb|#Sort|, and \verb|#Pattern| in Kore respectively.
